...
Local cMsgFci		:= "" 
Local cMensAux		:= ""
Local cVoperDoc	:= ""   // Variavel para informar o valor total da OperaÁ„o do doc Original conforme chamado TIGZGX
...
Local lCalSol		:= .F.
Local lOk			:= .T.
Local lBrinde		:= .F.							// Flag que define se È uma operaÁ„o de Brinde
Local lContinua	:= .T.
Local lCabAnf		:= .T.
...
Local lMvImpFecp    := SuperGetMV("MV_IMPFECP", ,.F.)	
Local lOrgaoPub	  := GetNewPar("MV_NFORGPU",.F.)				//NF-e de remessa nas operaÁıes de aquisiÁ„o de Ûrg„o p˙blico, com entrega em outro Ûrg„o p˙blico (RICMS SP)
...
If cTipo == "1"
	...
	If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
		...
		Else
			...
			If MsSeek(xFilial("SF3")+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE)
				...
				If lMvImpFecp
					...
					If (lValFecp .Or. lVfecpst)
						If SF3->F3_VALFECP > 0
				   	   		cMensFis += " BC R$: " + Alltrim(Str(SF3->F3_BASEICM, 14, 2)) + ", o adicional de " + Alltrim(Str(SFT->FT_ALQFECP, 14, 2))+"%" + " valor FECP R$ " + Alltrim(Str(SF3->F3_VALFECP, 14, 2))
					    Endif
						If SF3->F3_VFECPST > 0
				   			cMensFis += " BC R$: " + Alltrim(Str(SF3->F3_BASERET, 14, 2)) + ", o adicional de" + Alltrim(Str(SFT->FT_ALQFECP, 14, 2))+"%" + " valor FECP R$ " + Alltrim(Str(SF3->F3_VFECPST, 14, 2))
					    Endif
					Endif
				Endif
			...
			//-- para tes 799, n„o altera volume mesmo se for igual a 0
			//-- conforme solicitado pela LogÌstica (Cassiano)
			For _xE:=1 to Len(aEspVol)
				If (cAl)->(!eof())
					If alltrim((cAl)->D2_TES)#'799'
						aEspVol[_xE][2] := Iif(aEspVol[_xE][2]==0,1,aEspVol[_xE][2])
					Endif
				Else
					aEspVol[_xE][2] := Iif(aEspVol[_xE][2]==0,1,aEspVol[_xE][2])
				EndIf
			...
			If !Empty(SF2->F2_DUPL)	
				...
				#IFDEF TOP
					...
					BeginSql Alias cAliasSE1
						...
						SELECT E1_FILIAL,E1_PREFIXO,E1_NUM,E1_PARCELA,E1_TIPO,E1_VENCORI,E1_VALOR,E1_VLCRUZ,E1_ORIGEM,E1_PIS,E1_COFINS,E1_CSLL,E1_INSS,E1_VLRREAL,E1_IRRF
						FROM %Table:SE1% SE1
						WHERE
					...
					SF2->F2_DOC == (cAliasSE1)->E1_NUM     
						If (cAliasSE1)->E1_TIPO = MVNOTAFIS .OR. ((Alltrim((cAliasSE1)->E1_ORIGEM) $ 'LOJA701|FATA701|LOJA010') .AND. (cAliasSE1)->E1_TIPO $ cWhere)
							...
							If cValLiqB == "1"
								...
								,(nValDupl-(cAliasSE1)->E1_PIS-(cAliasSE1)->E1_COFINS-(cAliasSE1)->E1_CSLL-(cAliasSE1)->E1_INSS)-(cAliasSE1)->E1_IRRF})					
							Else
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD2
						...
						D2_QUANT,D2_TOTAL,D2_DESCON,D2_VALFRE,D2_SEGURO,D2_PEDIDO,D2_ITEMPV,D2_DESPESA,D2_VALBRUT,D2_VALISS,D2_PRUNIT,/* WHB Inicio*/D2_TP,/* WHB Fim */
			...
			While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				/* Tratamento para com base na legislaÁ„o do Estado do Paran· Decreto n 6.080/2012 - DOE PR Suplemento  
							...
							If MsSeek(xFilial("SD1")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
								...
								if AllTrim(SF1->F1_ESPECIE)<>"NFP"
									if cChave <> dToS( SD1->D1_EMISSAO ) + SD1->D1_SERIE + SD1->D1_DOC + iIf( SD1->D1_TIPO $ "DB", iIf( SD1->D1_FORMUL == "S", SM0->M0_CGC, SA1->A1_CGC ), iIf( SD1->D1_FORMUL == "S", SM0->M0_CGC, SA2->A2_CGC ) ) + SM0->M0_ESTCOB + SF1->F1_ESPECIE + SF1->F1_CHVNFE;
										...
										// Conforme consulta do chamado TIGZGX atribuido valor na variavel para montagem da frase
										cVoperDoc := SF1->F1_VALBRUT
									endIf	
						   		...
						   		If MsSeek(xFilial("SFT")+"S"+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI)
						   			If !Empty(SFT->FT_DTCANC)
								   		...
								   		If Empty(SF3->F3_CODRSEF) .Or. SF3->F3_CODRSEF == "101"
											...
											cItemOr	:= ( cAliasSD2 )->D2_ITEM
										endIf
									ElseIf SFT->FT_ESTADO == "EX" .or. (SubStr(SM0->M0_CODMUN,1,2) == "35" .and. "REMESSA POR CONTA E ORDEM DE TERCEIROS" $ Upper(cNatOper) .and. lOrgaoPub )//(Venda para orgao publico - SP/CFOP Remessa por conta e ordem de terceiros (cfop 5923/6923)- ch:TIDWCY   
	...
	//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
					...
					//impresso os valores da lei da transparÍncia.					
					if lNfCup
						cTpCliente := "F"
					EndIf
					...
					If !Empty(SF4->F4_FORMULA) .And. Formula(SF4->F4_FORMULA) <> NIL .And. ( ( cMVNFEMSF4=="C" .And. !AllTrim(Formula(SF4->F4_FORMULA)) $ cMensCli ) .Or. (cMVNFEMSF4=="F" .And. !AllTrim(Formula(SF4->F4_FORMULA))$cMensFis) )
						...
						EndIf
					// WHB Inicio incluido p/ trazer a descriÁ„o fiscal na danfe					
					Else 
					
						If !Alltrim(SF4->F4_DESCR)$cMensFis   //Incluido para adicionar somente uma vez
						   cMensFis += Alltrim(SF4->F4_DESCR)
					    Endif
				    // WHB Fim
					...
					//Verifica se existe Template DCL
      				IF (ExistTemplate("PROCMSG"))
      					cMensAux := ExecTemplate("PROCMSG",.f.,.f.,{cAliasSD2})      										 
   	     				If !(cMensFis$cMensAux)
	   	     				cMensFis += cMensAux
   						Endif     				
     				Endif 
					...
					*/					
					...
					MsSeek(xFilial("SFT")+"S"+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_ITEM+"  "+(cAliasSD2)->D2_COD) 
		            ...
		            nIcmsST := 0
		            If (!lIcmSTDev .And. (cAliasSD2)->D2_TIPO == "D" .And. SubStr((cAliasSD2)->D2_CLASFIS,2,2) $ '10#30#70#90') .Or. (Alltrim((cAliasSD2)->D2_CF) $ cMVCFOPREM) .Or. (!lIcmSTDev .And. lComplDev .And. (cAliasSD2)->D2_TIPO == "I" )
					...
					//⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒø
					//≥Busca o codigo do cliente SA7 - inserido por Alexandre R. Bento 18/03/09≥
					//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ	            
	
					If Subs((cAliasSD2)->D2_COD,11,1) == "." .Or. Subs((cAliasSD2)->D2_COD,13,1) == "." .Or. Subs((cAliasSD2)->D2_COD,1,2) == "EC" .Or. (cAliasSD2)->D2_TP == "PA" .Or. (cAliasSD2)->D2_TIPO$"N/C/I/P" 
		         
			           IF SA7->(DbSeek(xFilial("SA7")+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD)) 
				          _cCod    := Alltrim(SA7->A7_CODCLI)  //Pega o codigo do cliente  
				          _cPCFor  := Alltrim(SA7->A7_PCCLI)  // Numero do pedido do cliente na WHB (New Hubner)
				 	      _cCodFor := Alltrim(SA7->A7_CODFOR)  // Codigo da New Hubner no Cliente
	  			          IF !Empty(Alltrim(SA7->A7_PCCLI)) /*.And. (cAliasSD2)->D2_CLIENTE$"900025/900022/900171/900024"*/ .And. ((cAliasSD2)->D2_TP == "PA" .Or. (cAliasSD2)->D2_TES == "578") //gera a tag XML (NPED, NITENS) somente p/ PA.  
				             aadd(aPedCom,{Alltrim(SA7->A7_PCCLI),Iif(Empty(SA7->A7_NLINPED),1,SA7->A7_NLINPED)}) //nf saida pesquisar SA7 numero do pedido + item do pedido
	            		      cMensCli += " Ped.Compra: " +Alltrim(SA7->A7_PCCLI)  // InformaÁ„o do fisco			           
	            		      If (cAliasSD2)->D2_TP == "PA" .Or. (cAliasSD2)->D2_TES == "578"//Para tratar a TAG InfAdProd
	            		         _cCodcli := _cCod
	            		         _cPC     := _cPCFor
	            		       
	            		         If SM0->M0_CODIGO$"FN/NH"
	            		         	While(Len(_cCodcli) < 11) // codigo do cliente deve ter 11 caracteres
	            		               _cCodcli := "0"+_cCodcli
	            		         	Enddo
	            		         	While(Len(_cPC) < 12) // codigo do pedido deve ter 12 caracteres
	            		               _cPC := "0"+_cPC
	            		         	Enddo
	            		         Endif	
	            		         
	            		         If SM0->M0_CODIGO$"FN/NH" //Gera a tag IAP adicional p/ todos os clientes
	            		            cInfAdic +=IIf(!Empty(cInfAdic)," ","")+"IAP01"+ Iif(len(_cCodcli)>11, _cCodcli+Space(30-len(_cCodcli)),_cCodcli+Space(19))+_cPC
		            		        If (cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA== "90002201" //Cliente IVECO gera TAG com MOTORS
		             		           cInfAdic += "MOTORES   "
		            		        Endif
	            		         Else //Itesapar
	            		         
	            		            // Retira os Pontos do codigo do produto
							        _nAux := AT(".",_cCodcli)
							        While _nAux <> 0
		       
	    						       _cCodcli := Stuff(_cCodcli,_nAux,1,"")   
							           _nAux := AT(".",_cCodcli)
	           
							        Enddo 
	            		            cInfAdic +=If(!Empty(cInfAdic)," ","")+ "Cod. Ped.: "+Alltrim(_cPC) +" - Fabricacao: " +StrZero(Day(ddatabase),2)+'/'+StrZero(Month(ddatabase),2)+'/'+StrZero(Year(ddatabase),4) + " - Cod. Cliente: "
	            		            If (cAliasSD2)->D2_CLIENTE$ "900077" //Cliente Bosch 
	            		               cInfAdic += "00010"+ Alltrim(_cCodcli)            		            
	            		            Else
	            		               cInfAdic += "00001"+ Alltrim(_cCodcli)            		            
	            		            Endif   
		            		     Endif
	            		      Endif   
	 	                  Else
				          ...
				          Endif   
	  			          ...
	  			          IF !Empty(Alltrim(SA7->A7_CODFOR))   
	            		      cMensCli += " Cod.Forn: " +Alltrim(SA7->A7_CODFOR) // Codigo da WHB no Cliente
				          Endif   
				 	...
				 	Else
	                   aadd(aPedCom,{})                  
					   _cCod    := (cAliasSD2)->D2_COD //atribui o codigo do produto                   
				 	Endif       
				 	
		            If SA1->(DbSeek(xFilial("SA1")+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA))
		               If !Empty(SA1->A1_DUNS)                                 
	           		      cMensCli += " DUNS: " +Alltrim(SA1->A1_DUNS)   // Utilizado p/ impressao na nota fiscal È obrigatorio p/ GM
	            	   Endif	   
							If SM0->M0_CODIGO == "IT"						
								If SA1->A1_COD+SA1->A1_LOJA $ '90008601'
									cMensCli += "Dados para pagamento desta Nota Fiscal: Paran· Banco - 254 Ag. 0001 C/C 14705-9"
								Endif
							Endif
		            Endif
					...
					If (SF4->(FieldPos("F4_CONSUMO")) > 0 .And. SF4->F4_CONSUMO == "N") .And. (cOrigem $"1-2-3-4-5-6-8" .And. cCSTrib $ "00-10-20-40-60-70-90")    /*Inserido o cÛdigo 060 conforme consulta do chamado TIDWAX */
					...
					aadd(aProd,	{Len(aProd)+1,;
						/* WHB INICIO */ _cCod,; // cCodProd,; //-- WHB FIM
						...
						IIF((cAliasSD2)->(FieldPos("D2_TOTIMP"))<>0,(cAliasSD2)->D2_TOTIMP,0),;   //Total imposto carga tribut·ria. 
						(cAliasSD2)->D2_DESCZFP,;			//Desconto Zona Franca PIS
						(cAliasSD2)->D2_DESCZFC,;			//Desconto Zona Franca CONFINS
						(cAliasSD2)->D2_PICM	})			//Percentual de ICMS
		    ...
		    //Mensgaem para ICMS Particionado - ConvÍnio ICMS N∫ 51/00,
		    if nValICMParc > 0 .And. nBasICMParc > 0 .And. nValSTParc > 0 .And. nBasSTParc > 0
				...
				If !Empty(aEntrega) 
					...
					cMensFis += "EndereÁo: "+ConvType(aEntrega[02],125)+", "+ConvType(aEntrega[03],10)+" "+ConvType(aEntrega[04],60)+". " //Rua,Num,Complemento
				...
				Else
					...
					cMensFis += "EndereÁo: "+ConvType(aDest[03],125)+" "+ConvType(aDest[04],10)+" "+ConvType(aDest[05],60)+", " //Rua,Num,Complemento
		    ...
		    if lBrinde
		    	...
		    	cMensCli += "Nota Fiscal emitida nos termos do inciso II do art. 456 do RICMS - Nota Fiscal de AquisiÁ„o n∫ "+aDocDat[2]+", de "+aDocDat[1]+"."
		    endif 
		EndIf
	EndIf
Else
		...
		Else				
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD1			
						...
						/* WHB INICIO */ ,D1_DESCRI,D1_NCM /* WHB FIM */
						FROM %Table:SD1% SD1
						WHERE
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				EndIf
	   			...
	   			//Verifica se existe Template DCL
      			IF (ExistTemplate("PROCMSG"))
      				cMensAux := ExecTemplate("PROCMSG",.f.,.f.,{cAliasSD1})      										 
   	     			If !(cMensFis$cMensAux)
	   	     			cMensFis += cMensAux
   					Endif     				
     			Endif
				...
				If !Empty((cAliasSD1)->D1_NFORI)
					...
					Else
						...
						ELSE 
							if !(SF4->F4_AJUSTE=='S' .and. ((cAliasSD1)->D1_TIPO == "D"))
					   		...
					   		else  /* Quando for uma devoluÁ„o de Ajuste n„o tem necessidade de informar os outros campo para posicionar na SD2. chamado TIANDL*/
					   			cMsSeek	:=(xFilial("SD2")+(cAliasSD1)->D1_NFORI+(cAliasSD1)->D1_SERIORI)
					   		endif       
						EndIF                                                                          
				...
				aadd(aProd,	{Len(aProd)+1,;  
					(cAliasSD1)->D1_COD,;
					IIf(Val(SB1->B1_CODBAR)==0,"",StrZero(Val(SB1->B1_CODBAR),Len(Alltrim(SB1->B1_CODBAR)),0)),;
					/* WHB INICIO */ (cAliasSD1)->D1_DESCRI,; // SB1->B1_DESC,; //-- WHB FIM
					/* WHB INICIO */ (cAliasSD1)->D1_NCM,; // SB1->B1_POSIPI,; //-- WHB FIM	
					SB1->B1_EX_NCM,;
					(cAliasSD1)->D1_CF,;
					SB1->B1_UM,;
					(cAliasSD1)->D1_QUANT,;
					...
					"",;
					0,;
					0,;  // Da posiÁ„o 28 a 30 tratamento realizado apenas para documento de saÌda por este motivo campos est„o zerados e vazios.
					0,;			//Desconto Zona Franca PIS
					0,;			//Desconto Zona Franca CONFINS
					0})			//Percentual de ICMS
...
If !Empty(aNota)
	cString := ""
	cString += NfeIde(@cNFe,aNota,cNatOper,aDupl,aNfVinc,cVerAmb,aNfVincRur,aRefECF,cVoperDoc)
	...
	For nX := 1 To Len(aProd)
		If nLenaIpi > 0/* WHB INICIO */ .And.  nX <= nLenaIpi //-- WHB FIM
			If  nCstIpi <= nLenaIpi
		...
		cString += 	NfeItem(aProd[nX],aICMS[nX],aICMSST[nX],aIPI[nX],aPIS[nX],aPISST[nX],aCOFINS[nX],aCOFINSST[nX],aISSQN[nX],aCST[nX],;
					...
					@cMensFis,aCsosn[Nx],aPedCom[nX],aNota[5],aICMSZFM[nX],aDest,cIpiCst,aFCI[nX],lIcmDevol)
	Next nX
  	cString += NfeTotal(aTotal,aRetido,aICMS,aICMSST,lIcmDevol)
	cString += NfeTransp(cModFrete,aTransp,aImp,aVeiculo,aReboque,aEspVol,cVerAmb,aReboqu2)
	cString += NfeCob(aDupl)
	cString += NfeInfAd(cMensCli,cMensFis,aPedido,aExp,cAnfavea,aMotivoCont,aNota,aNfVinc,aProd,aDI,aNfVincRur,aRetido,cNfRefcup,cSerRefcupc,cTipo,nIPIConsig,nSTConsig,lBrinde,cVoperDoc/*WHB INI*/,aTransp/*WHB FIM*/)
	cString += "</infNFe>"
EndIf
Return({cNfe,EncodeUTF8(cString),cNotaOri,cSerieOri})

Static Function NfeIde(cChave,aNota,cNatOper,aDupl,aNfVinc,cVerAmb,aNfVincRur,aRefECF,cVoperDoc)

Local cString    := ""
Local cNFVinc    := ""
Local cModNot    := ""
Local cTPNota    := ""
Local cMVCfopTran	:= SuperGetMV("MV_CFOPTRA", ," ")   		// Parametro que define as CFOP¥s pra transferÍncia de CrÈdito/DÈbito
...
If cVerAmb== "2.00" 
	Do Case
		...
		Case ((aNota[5]) $ "I-D-C" .And. SF4->F4_AJUSTE == "S")
	   		cTPNota:= "3"
		/* Referente ao chamado TIDMJV que contempla, nota de transferÍncia de crÈdito / dÈbito */	   		
...
Static Function NfeItem(aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ,aAnfI,cTipo,cVerAmb,aComb,cMensFis,cCsosn,aPedCom,cF2Tipo,aICMSZFM,aDest,cIpiCst,aFCI,lIcmDevol)
		...
		If aCST[01] = "60" .AND. cTipo=="1"		
			...
			If nBaseIcm > 0 .and. nValICM>0
				...
				ElseIF SF4->F4_ART274 == "1"  .And. Upper(SM0->M0_ESTCOB) == "PR"  
					nBaseIcm := aProd[09]*nBaseIcm
					nValICM  := aProd[09]*nValICM
					if SF4->F4_CODIGO > "500"  /* TES de SaÌda */  
						cMensFis += " Imposto Recolhido por SubstituiÁ„o - Artigo 2., II , Anexo X , do RICMS/PR Decreto 6080/2012, onde o 'Cod.Produto:  " +ConvType(aProd[02])+" '  Valor da Base de ST: R$ " +str(nBaseIcm,15,2)+" Valor de ICMS ST: R$ "+str(nValICM,15,2)+" "
					else //entrada
						cMensFis += " Imposto Recolhido por SubstituiÁ„o - Artigo 4., II , Anexo X , do RICMS/PR Decreto 6080/2012, onde o 'Cod.Produto:  " +ConvType(aProd[02])+" '  Valor da Base de ST: R$ " +str(nBaseIcm,15,2)+" Valor de ICMS ST: R$ "+str(nValICM,15,2)+" "
					endif
				...
				ElseIF SF4->F4_ART274 == "1"  .And. Upper(SM0->M0_ESTCOB) == "AM"
				 	...
				 	If (lNContrICM .And. SA1->A1_EST <> "AM") .Or. SA1->A1_EST == "AM"  //Conforme consulta (TGVUIP).
				 		cMensFis += "Mercadoria j· tributada nas demais fases de comercializaÁ„o - ConvÍnio ou Protocolo ICMS n∫ "+Alltrim(aProd[28])+ ". Cod.Produto: " +ConvType(aProd[02])+"."
				 	EndIf
				...
				ElseIF SF4->F4_ART274 == "1"  .And. Upper(SM0->M0_ESTCOB) == "RS"			 
					...
					nValICM  := aProd[09]*nValICM
				 	if !Empty(aProd[28])				 	
				 		cMensFis += "Imposto recolhido por ST nos termos do (ConvÍnio ou Protocolo ICMS n∫ "+ Alltrim(aProd[28]) +") RICMS-RS. Valor da Base de ICMS ST R$"+ cValToChar(nBaseIcm) +" e valor do ICMS ST R$ "+ cValToChar(nValICM) +". Cod.Produto: " +ConvType(aProd[02])+"." 
				 	else
				...
				ElseIf SF4->F4_ART274 == "1"  .And. Upper(SM0->M0_ESTCOB) == "MG" .And. Len(aICMS) > 0 .And. ( nBaseIcm+nValICM > 0 )  // Conforme Chamado TIABCS
					...
					cMensFis += "Imposto Recolhido por SubstituiÁ„o - ICMS retido pelo cliente S.T. DECRETO 43708 19/12/2009."
...
/*Nota TÈcnica 004 de 2011 conforme chamado - THCTB4 */

if lSuframa .and. Len(aICMSZFM)>0 .and. aCST[1] $ '40' 
...
If aProd[30] > 0 .And. cMvMsgTrib $ "2-3" .And. cTpCliente == "F"
	cCrgTrib := 'Valor Aproximado dos Tributos: R$ '+ ConvType(aProd[30],15,2)+'.'
EndIf
...
Static Function NfeTotal(aTotal,aRet,aICMS,aICMSST,lIcmDevol)
...
Static Function NfeInfAd(cMsgCli,cMsgFis,aPedido,aExp,cAnfavea,aMotivoCont,aNfSa,aNfVinc,aProd,aDI,aNfVincRur,aRet,cNfRefcup,cSerRefcup,cTipo,nIPIConsig,nSTConsig,lBrinde,cVoperDoc/*WHB INI*/,aTransp/*WHB FIM*/)
...
If Len( aNfVinc ) > 0		//Nota de espÈcie NFE e CTE vinculada  
	For nZ := 1  to Len( aNfVinc )
		If !( aNfVinc[nZ][2] + aNfVinc[nZ][3] ) $ cChvNFe
			...
			if SM0->M0_ESTENT == 'SP'  // Conforme chamado TIGZGX feito com a Consultoria tributaria
				if !Empty(cVoperDoc)
					cString += "Valor da Operacao do Documento de Origem: " + ConvType(cVoperDoc,15,2) +"."
				endif			
			endif 
		EndIf
	Next nZ
...
// WHB Inicio                   
//900047-01 cliente WEG Itesapar
If SM0->M0_CODIGO$"IT" .And. cTipo=="1" //somente nota de saida p itesapar

	cString += "CHEGADA: " +dtoc(ddatabase+1)+" 16:00 " 
    If Len(aTransp[01])==14
       cString += " REDESPACHO: " +  ConvType(aTransp[02])  + " CGC: " + aTransp[01] + " INSCRICAO ESTADUAL: "+ aTransp[03] +  " CIDADE: " + ConvType(aTransp[05])
	Endif  
...
If cMvMsgTrib $ "1-3" .And. nTotalCrg > 0 .And. cTpCliente == "F"
	If !Empty(cFntCtrb)
		cString += 'Valor Aproximado dos Tributos: R$ ' +ConvType(nTotalCrg,15,2)+'. Fonte:'+cFntCtrb+'.'
	Else
		cString += 'Valor Aproximado dos Tributos: R$ ' +ConvType(nTotalCrg,15,2)+'.'
	EndIf 
EndIf
...
If lIcmsSemSt .And. lIcmsST
	...
	cMsg += "OperaÁıes n„o sujeitas a Regime de ST, "
	cMsg += "Base de C·lculo do ICMS prÛprio: R$ " + Alltrim( Str(nBaseIcm-nBaseST, 14, 2) )+ ", "
	cMsg += "Valor do ICMS prÛprio: R$ " + Alltrim( Str(nValIcm-nValST, 14, 2) )+ ". "
	cMsg += "OperaÁıes sujeitas a Regime de ST, " 			
	cMsg += "Base de c·lculo do ICMS prÛprio : R$ " + Alltrim( Str(nBaseST, 14, 2) )+ ", "
	cMsg += "Valor do ICMS prÛprio: R$ " + Alltrim( Str(nValST, 14, 2) )+ ". "
...
Funcao criada para retornar para a funÁ„o XmlNfeSef os valores da Nota Original quando houver controle de SubLote
