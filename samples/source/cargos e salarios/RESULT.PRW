...
If nOpca == 1
	...
	dbSelectArea("RB0")
	dbSetOrder(1)
	cFil := If(xFilial("RB0") == "  ",xFilial("RB0"),cAuxFil)
	...
	dbSelectArea("RB4")   
	dbSetOrder(1)
	cFil := If(xFilial("RB4") == "  ",xFilial("RB4"),cAuxFil)
...
dbSelectArea("RB1")
dbSetOrder(1)
cFil := If(xFilial("RB1") == "  ",xFilial("RB1"),cAuxFil)
...
dbSelectArea("RB5")
dbSetOrder(1)
cFil := If(xFilial("RB5") == "  ",xFilial("RB5"),cAuxFil)
...
Else      
	...
	nTam	:= 4	
	cTitulo := "Empresas Convidadas na Pesquisa"
	For i :=1 To Len(aAux)    
		dbSelectArea("RB0")
		dbSetOrder(1)
...
_cPath := cGetFile( "Impressao Resultado da Pesquisa Salarial (.DOT) | ",OemToAnsi("Selecione Diretorio"),,"",.F.,GETF_RETDIRECTORY)	
