...
Begin Sequence
   ...
   Else
      ...
      cDTVENCTO := IncSpace(SY6->(MSMM(SY6->Y6_DESC_P,60,1)),60,.f.)
      ...
      cMunBco  := ALLTRIM(SA6->A6_MUN)+", "+ALLTRIM(SA6->A6_UNIDFED)+" "+ ALLTRIM(SA6->A6_CEP)
   ...
   DO WHILE ! EEB->(EOF()) .AND.;
      ...
      IF LEFT(EEB->EEB_TIPOAG,1) = "3"  // Recebedor de Comissao
         ...
         IF EEC->EEC_TIPCVL = "1" .OR. EEC->EEC_TIPCVL = "3"
            ...
            DETAIL_P->AVG_C01_20 := ALLTRIM(TRANSFORM((EEB->EEB_TXCOMI/100)*nFob,"@E 999,999,999.99"))
         ELSE
            DETAIL_P->AVG_C01_20 := ALLTRIM(TRANSFORM(EEB->EEB_TXCOMI,"@E 999,999,999.99"))
         ENDIF
...
Begin Sequence
   ...
   IF EMPTY(cEXPMEMO)
      ...
      cEXPMEMO := cEXPMEMO+ALLTRIM(If(!Empty(cEXP_FONE),STR0043,"")+AllTrim(cEXP_FONE))+If(!Empty(cEXP_FAX),STR0044+AllTrim(cEXP_FAX),"") //"TEL.: "###" FAX: "
   ENDIF
