...
WHILE .T.
  ...
  DEFINE MSDIALOG oDlg TITLE STR0001 FROM 8, 0 TO  25, 80 //"Emissão da Comercial Invoice"

  nLin   := 30.5
  nCol   := 10.0
  nOpcao := 0

  @ nLin , nCol       SAY STR0002 SIZE  35, 8 //"Conhecimento"
  @ nLin , nCol+40.0  GET cHouse  F3 "SW6" PICTURE "@!" VALID PO558Val() SIZE  70, 8
...
nLi_Fim2 := 0
//---------> D.I. (Itens).
SW7->( DbSeek( xFilial("SW7")+SW6->W6_HAWB ) )  //Declaracao

//---------> P.O. (Capa).
SW2->(DbSetOrder(1))
SW2->( DbSeek( xFilial("SW2")+SW7->W7_PO_NUM ) ) //It_Declaracao
...
cEndeImport := cEndeImport + If( !Empty(SYT->YT_PAIS  ), ALLTRIM(SYT->YT_PAIS  )+" - ", "")
...
AVPRINT oPrn NAME STR0009 //"Commercial Invoice"
   ...
   AVPAGE
        ...
        DO WHILE ! SW7->(EOF()) .AND. SW7->W7_FILIAL == xFilial("SW7") .AND. SW7->W7_HAWB == cHouse
           ...
           If !Empty( SW7->W7_PGI_NUM )
              If SubStr(Alltrim(SW7->W7_PGI_NUM),1,1) #"*"
                 Aadd( aPgis, ALLTRIM(SW7->W7_PGI_NUM) )
              Endif
           Endif
           ...
           SYG->(DBSEEK(xFilial("SYG")+SW2->W2_IMPORT+SW7->W7_FABR+SW7->W7_COD_I))
           mDescri := ''
           ...
           oPrn:Say( nLinha ,430  ,TRANS(SW7->W7_QTDE,AVSX3("W7_QTDE",6)),aFontes:TIMES_NEW_ROMAN_12,,,,1)
           oPrn:Say( nLinha ,500  ,MEMOLINE(mDescri,35,1),aFontes:TIMES_NEW_ROMAN_12)
           oPrn:Say( nLinha ,1710 ,trans(SW7->W7_PRECO*SW2->W2_PARID_U,AVSX3("W7_PRECO",6)),aFontes:TIMES_NEW_ROMAN_12,,,,1)
           oPrn:Say( nLinha ,2180 ,trans(SW7->W7_QTDE*(SW7->W7_PRECO*SW2->W2_PARID_U),AVSX3("W7_PRECO",6)),aFontes:TIMES_NEW_ROMAN_12,,,,1)

           nFob_Tot := nFob_Tot + ( SW7->W7_QTDE*(SW7->W7_PRECO*SW2->W2_PARID_U) )

           IF EMPTY(nPari_1)
              nPari_1:= SW2->W2_PARID_U
           ENDIF
           nNetWeight := nNetWeight + (SW7->W7_QTDE*W5Peso() )// FCD 04/07/01
           ...
           nLi_Fim:=nLinha
           PO558VerFim(1)
           oPrn:Say( nLinha,500  ,STR0012+alltrim(trans(SW7->W7_QTDE*W5Peso(),AVSX3("B1_PESO",6)+STR0013)), aFontes:TIMES_NEW_ROMAN_12) //"PESO LIQUIDO:  "###" KGS" //FCD 04/07/01
           nLinha := nLinha + 50
           oPrn:Line( nLinha , 110, nLinha  , 2240 )
           oPrn:Line( nLinha+1,  110, nLinha+1, 2240 )

           IF ASCAN(aVetor,SW7->W7_FABR) == 0
              AADD(aVetor,SW7->W7_FABR)
           ENDIF
        ...
        oPrn:Say( nLinha ,2180 ,trans(nFob_Tot,AVSX3("W6_FOB_TOT",6)), aFontes:TIMES_NEW_ROMAN_12 ,,,,1)
        PO558VerFim(2)
        nLinha := nLinha + 50
        ...
        PO558VerFim(0)
        nLinha := nLinha + 90
        oPrn:Say( nLinha ,110  ,STR0025+ trans(nNetWeight,AVSX3("B1_PESO",6))+STR0013, aFontes:TIMES_NEW_ROMAN_12) //"NET WEIGHT: "###" KGS"
        PO558VerFim(0)
        nLinha := nLinha + 90
        oPrn:Say( nLinha ,110  ,STR0026 + Trans(SW6->W6_PESO_BR,AVSX3("W6_PESO_BR",6)), aFontes:TIMES_NEW_ROMAN_12) //"GROSS WEIGHT: "
        PO558VerFim(0)
        nLinha := nLinha + 90
...
oPrn:Say( nLinha     , 1480,STR0035 ,aFontes:TIMES_NEW_ROMAN_12) //"UNIT US$"
oPrn:Say( nLinha     , 1880,STR0036 ,aFontes:TIMES_NEW_ROMAN_12) //"TOTAL US$"
nLinha := nLinha + 50
oPrn:Line( nLinha    , 110, nLinha  , 2240 ) 
oPrn:Line( nLinha+1,  110, nLinha+1, 2240 )
