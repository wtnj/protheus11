...
±±³Fun‡…o    ³ ECOIV200 ³ Autor ³ VICTOR IOTTI          ³ Data ³ 21.04.99 ³±±
...
lAtuDtCont:=MsgYesNo(STR0001) //"Integração Efetiva ?"
MsAguarde({|| E_Open(bMSG,aARQ_EIC,@lAbre) },STR0002) //"Abertura de Arquivos"
...
While .T.
   IF !(lStatus:=Pergunte( "EICIV2", .T. ))  .OR.  ! U_IV_VALID("*")
      IF !lStatus
          lRetFunc:=.F.
          Exit
      ENDIF
...
If lRetFunc
   ...
   If lExisteECF .And. lExisteECG
      Processa({||ApagaECFECG()},STR0003)   //'Aguarde... Atualizando Dados'
      Processa({||GeraPagtos()}, STR0004) //'Aguarde... Apurando Pagamentos Antecipados'
   Endif

   Processa({|| IV200GrvDI() })
   Processa({|| IV200GrvAPD() })

   E_Msg(AnsiToOem(STR0005),1) //"Geração concluída. Clique OK para continuar ..."
EndIf
...
//          EndIf         
      ...
      IF !SW2->W2_IMPORT $ MImport  && SELECAO DE IMPORTADOR
         SW8->( DBSKIP() )
         LOOP
      ENDIF
      ...
      cImpCGC:="01"
      IF SYT->(DbSeek(xFilial()+SW2->W2_IMPORT))
         cImpCGC:=SUBSTR(SYT->YT_CGC,11,2)
      Endif

      IF ! SW9->( DBSEEK(xFilial()+SW8->W8_INVOICE+SW8->W8_FORN) )
         E_MSG( STR0007 + SW8->W8_INVOICE + STR0008+ SW8->W8_FORN + STR0009,1000 ) //### //"INVOICE SEM HEADER "###" - "###" FAVOR ANOTAR E TECLAR ENTER"
         SW8->( DBSKIP() )
         LOOP
      ENDIF
...
DO WHILE .NOT. SWB->(EOF()) .AND. SWB->WB_CA_DT <= (dFimEncerra + 5) ;  && pagto no mes
   ...
   IF SWB->WB_DT_DESE < dIniEncerra .OR. SWB->WB_DT_DESE > dFimEncerra
      SWB->(DBSKIP())
      LOOP
   ENDIF
   ...
   IF  ! lDI100 .And. !lPagAnt
       ...
       DO WHILE !SW8->(EOF()) .AND. SW6->W6_HAWB = SW8->W8_HAWB
                                    cFil_SW8     = SW8->W8_FILIAL
          
          SW9->( DBSETORDER(1) )
          IF ! SW9->( DBSEEK(xFilial()+SW8->W8_INVOICE+SW8->W8_FORN) )
             MsgInfo( STR0010 + SW8->W8_INVOICE + STR0008+ SW8->W8_FORN + STR0011,STR0012)//### //"INVOICE SEM CAPA "###" - "###" FAVOR ANOTAR"###"Informação"
             SW8->( DBSKIP() )
             LOOP
          ENDIF
   ...
   SYS->(dbSeek(cFil_SYS+"I"+TIPO_INVOICE+SWB->WB_HAWB+SWB->WB_FORN+SWB->WB_MOEDA+SWB->WB_INVOICE))
   lEntrou := .F.
   aTotal:={0,0,0,0,0}
...
//    ENDCASE
      ...
      Do While SWB->(!EOF()) .And. SWB->WB_FILIAL == xFilial("SWB") .And.;
          ...
          If SWB->WB_INVOICE == SW9->W9_INVOICE .And. SWB->WB_TIPOREG=="1"
             ...
             If ECF->(DBSEEK(cFilECF+SW9->W9_FORN+SWB->WB_INVOICE+AVKEY(SYS->YS_CC,"ECF_IDENTC")+"608"+SWB->WB_LINHA+"PR"))
   ...
   IF lEntrou .AND. nTotal # 0 .and. nTotal #(SW9->(W9_FOB_TOT+W9_INLAND+W9_PACKING-W9_DESCONTO+W9_FRETEINT)-nTotAntecipado)
      SYS->(DBSKIP(-1))
      nTotal :=  nTotal-IV_Ctb->IVFOB_TOT
...
IF PArq == "2"
   ...
   MfreteInt := 0
   IF MPrimeiro
      MfreteInt := SW9->(W9_INLAND+W9_PACKING-W9_DESCONTO+W9_FRETEINT) / SW8->W8_QTDE  && rateio frete intl so'no 1 item
      MPrimeiro := .F.
   ENDIF

// If (SW9->(W9_FOB_TOT+W9_INLAND+W9_PACKING-W9_DESCONTO+W9_FRETEINT)-nTotAntecipado) > 0
   RecLock("IVH_Ctb",.T.)
...
AADD(aDBF_APD,{"APDBANCO  ","C",  4,0}) 
AADD(aDBF_APD,{"APDAGENCIA","C",  7,0}) 
...
If lExisteECF
   ...
   AADD(aANT_ANT,{"ANTBANCO  ","C",  4,0}) 
   AADD(aANT_ANT,{"ANTAGENCIA","C",  7,0}) 
...
Do While .T.
   ...
   IF ! USED()
      MSGALERT(STR0013+cPathCont+"DI100",STR0014) //"Não foi possivel abrir o arquivo "###"Atenção"
      lRet20B:=.F.
   ENDIF
   ...
   IF ! USED()
      DI_Ctb->(DbCloseArea())
      MSGALERT(STR0013+cPathCont+"IVH100",STR0014) //"Não foi possivel abrir o arquivo "###"Atenção"
      lRet20B:=.F.
   ENDIF
// IndRegua("IVH_Ctb",cPathCont+("IVH100"+OrdBagExt()),"IVHINVOICE+IVHIDENTCT")
   ...
   IF ! USED()
      DI_Ctb->(DbCloseArea())
      IVH_Ctb->(DbCloseArea())
      MSGALERT(STR0013+cPathCont+"IV100",STR0014) //"Não foi possivel abrir o arquivo "###"Atenção"
      lRet20B:=.F.
   ENDIF
   ...
   IF ! USED()
      ...
      IVH_Ctb->(DbCloseArea())
      IV_Ctb->(DbCloseArea())
      MSGALERT(STR0013+cPathCont+"APD100",STR0014) //"Não foi possivel abrir o arquivo "###"Atenção"
      lRet20B:=.F.
   ENDIF
   ...
   If lExisteECF
      ...
      IF ! USED()
         ...
         IV_Ctb->(DbCloseArea())
         APD_Ctb->(DbCloseArea())
         MSGALERT(STR0013+cPathCont+"ANT100",STR0014) //"Não foi possivel abrir o arquivo "###"Atenção"
         lRet20B:=.F.
      ENDIF                                                
...
IF cCampo == "*" .Or. cCampo == "DTINI"
   IF dIniEncerra > dFimEncerra
      MsgInfo( STR0015,STR0012) //"DATA INICIAL MAIOR QUE A DATA FINAL"###"Informação"
...
IF cCampo == "*" .Or. cCampo== "IMPOR1"
   IF !EMPTY(cImport1)
      IF  ! SYT->(DbSeek(xFilial()+cImport1))
          MsgInfo(STR0016,STR0012) //"IMPORTADOR NAO CADASTRADO"###"Informação"
          RETURN IF(cCampo=="*",.F.,.T.)
      ENDIF
   ENDIF
ENDIF
...
IF cCampo == "*" .Or. cCampo== "IMPOR2"
   IF !EMPTY(cImport2)
      IF ! SYT->(DbSeek(xFilial()+cImport2))
         MsgInfo(STR0016,"Informação") //"IMPORTADOR NAO CADASTRADO"
         RETURN IF(cCampo=="*",.F.,.T.)
      ENDIF
   ENDIF
ENDIF
...
IF cCampo == "*" .And. Empty(cImport1+cImport2+cImport3+cImport4)
   MsgInfo(AnsiToOem(STR0017),STR0012) //"Não há importadores selecionado"###"Informação"
   RETURN (.F.)
ENDIF
...
*------------------------------*         
Local nOrdSW9 := SW9->(IndexOrd()), nRecnoSW9 := SW9->(Recno()), aFornMoeda := {}, nPos := 0         
...
//    ENDCASE
      ...
      Do While SW9->W9_FILIAL = SW9->(xFilial()) .And. SW6->W6_HAWB == SW9->W9_HAWB
         ...
         If nPos > 0
            ...
            Endif
         Else
            AAdd( aFornMoeda, {SW9->W9_FORN, SW9->W9_MOE_FOB, SW9->W9_FOB_TOT, SW9->W9_INLAND, SW9->W9_FRETEINT, SW9->W9_PACKING, SW9->W9_DESCONTO, SW9->W9_TX_FOB } )
         Endif   
      ...
      DO WHILE SYS->(!EOF()) .AND. SYS->YS_TPMODU == "I" .AND. TIPO_HOUSE+SW6->W6_HAWB == SYS->YS_TIPO+SYS->YS_HAWB .AND. SYS->YS_FILIAL == cFil_SYS
         ...
         Do While SWB->(!EOF()) .And. SWB->WB_FILIAL == xFilial("SWB") .And.;
            ...
            If SWB->WB_INVOICE == SW9->W9_INVOICE .And. SWB->WB_TIPOREG=="1"
               nTotAntecipado += SWB->WB_PGTANT
               If lExisteECF .And. ECF->(DBSEEK(cFilECF+SW9->W9_FORN+SWB->WB_INVOICE+AVKEY(SYS->YS_CC,"ECF_IDENTC")+"608"+SWB->WB_LINHA+"PR"))
                  nTotAntecipado += SWB->WB_FOBMOE
               Endif    
            EndIf
            SWB->(dbSkip())
   ...
   //          DI_Ctb->DILOTE      := cLote              && LAB 24.08.00
               ...
               DI_Ctb->DIDT_ENCE   := SW6->W6_DT_ENCE
...
RETURN .T.
*-----------------------------*
FUNCTION ApagaECFECG()            
*-----------------------------*
ProcRegua(ECF->(Lastrec()+1))
ECF->(DBSETORDER(4))
ECG->(DBSETORDER(1))
ECF->(DbSeek(cFilECF+'9999'))
cHawb:=ECF->ECF_HAWB
...
DO WHILE ! ECF->(EOF()) .AND. ECF->ECF_FILIAL = cFilECF
   ...
   If ECF->(EOF()) .or. cHawb # ECF->ECF_HAWB .or. cOrigem # ECF->ECF_ORIGEM .or. cIdentc # ECF->ECF_IDENTC .or. cForn # ECF->ECF_FORN
      ECF->(DBSETORDER(3))
      If ECG->(DbSeek(cFilECG+cHawb+cForn+cIdentc+cOrigem)) .AND. ! ECF->(DbSeek(cFilECF+cHawb+cForn+cIdentc+cOrigem))
...
DO WHILE .NOT. SWB->(EOF()) .AND. SWB->WB_CA_DT <= (dFimEncerra + 5) ;  && pagto no mes
   ...
   If SWB->WB_PO_DI = 'A' .And. SWB->WB_TIPOREG = 'P'               
      ...
      DO WHILE SYS->(!EOF()) .AND. SYS->YS_FILIAL==cFil_SYS .AND. SYS->YS_TPMODU == "I" .AND. SYS->YS_TIPO+SYS->YS_HAWB == "P"+SWB->WB_HAWB
         ...
         If !ECG->(DbSeek(cFilECG+Avkey(SWB->WB_HAWB, "WB_HAWB")+cFornSW2+"PO"+SYS->YS_CC))
            Reclock("ECG",.T.)
...
Local nOrdemSWB := SWB->(IndexOrd()), nOrdemSA6 := SA6->(IndexOrd()),;
      nOrdemSW8 := SW8->(IndexOrd()), nOrdemSW9 := SW9->(IndexOrd()),;
...
IF SWB->(DBSEEK(cFil_SWB+SW6->W6_HAWB+"D")) .And. !Empty(SW6->W6_DT_ENCE) // Tipo de registro de pagamento antecipado para o embarque/desembaraco
   DO WHILE .NOT. SWB->(EOF()) .AND. SWB->WB_HAWB = SW6->W6_HAWB ;  
      ...
      If nPos = 0
         AADD(aInv, {cPo, SWB->WB_PGTANT, SWB->WB_LINHA, SWB->WB_INVOICE, cFornSW8} )  
      Else
       ...
       For j:=1  to Len(aCCInv)
           ...
           Do While ECF->(!Eof()) .And. aCCinv[j,1] = ECF->ECF_HAWB .And. ECF->ECF_IDENTC = aCCInv[j,4] .And. ECF->ECF_ORIGEM = "PO"                    
              ...
              If ECF->ECF_ID_CAM = '101' .Or. ECF->ECF_ID_CAM = '609'
                 ...
                 If nPos = 0
                    AAdd(aVlInv, {ECF->ECF_HAWB, ECF->ECF_SEQ, ECF->ECF_IDENTC, ECF->ECF_VL_MOE, ECF->ECF_VL_MOE, SW6->W6_DT_EMB, ECF->ECF_INVOIC})
                 Else
...
SA2->(DbSetOrder(1))
EC6->(DbSetOrder(1))
EC6->(DbSeek(xFilial()+"IMPORT"+cEvento+cCusto))

MCTA_DEB := EC6->EC6_CTA_DB
MCTA_CRE := EC6->EC6_CTA_CR
...
If cPoNum # NIL .And. cLinha # NIL .And. cEvento # NIL .And. cCusto # NIL                     
   If !ECF->(DbSeek(cFilECF+cPoNum+cFornec+cCusto+"PO"+cLinha+cEvento))
      RecLock("ECF",.T.)      
   Else
      RecLock("ECF",.F.)      
   Endif
