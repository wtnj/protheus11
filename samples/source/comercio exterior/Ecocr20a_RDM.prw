...
Do While .T.
   ...
   Processa({|| CR200Inv(),STR0001})// Substituido pelo assistente de conversao do AP5 IDE em 01/12/99 ==>    Processa({|| EXECUTE(CR200Inv),"Integrando registros de invoices."}) //"Integrando registros de invoices."
   Processa({|| CR200Des(),STR0002})// Substituido pelo assistente de conversao do AP5 IDE em 01/12/99 ==>    Processa({|| EXECUTE(CR200Des),"Integrando registros de despachos."}) //"Integrando registros de despachos."
   Processa({|| CR200ApaPag(),STR0062})  // Substituido pelo assistente de conversao do AP5 IDE em 01/12/99 ==>    Processa({|| EXECUTE(CR200Pag),"Integrando registros de pagamentos."})
   Processa({|| CR200Apa2Pag(),STR0063}) // Substituido pelo assistente de conversao do AP5 IDE em 01/12/99 ==>    Processa({|| EXECUTE(CR200Pag),"Integrando registros de pagamentos."})
   Processa({|| CR200Pag(),STR0064})     // Substituido pelo assistente de conversao do AP5 IDE em 01/12/99 ==>    Processa({|| EXECUTE(CR200Pag),"Integrando registros de pagamentos."})
...
If lExisteECF   
   ...
   DO WHILE ! ANT_Ctb->(EOF())      
      ...
      If ! ECF->(DBSEEK(cFilECF+ANT_Ctb->ANTHAWB+ANT_Ctb->ANTFORN+ANT_Ctb->ANTIDENTCT+ANT_Ctb->ANTORIGEM+ANT_Ctb->ANTINVOICE+ANT_Ctb->ANTSEQ+"608"))  // evitar duplicidade       
      
         EC6->(DbSeek(xFilial()+"IMPORT"+"608"+ANT_Ctb->ANTIDENTCT))
...
If !ECG->(DbSeek(cFilECG+ANT_Ctb->ANTHAWB+ANT_Ctb->ANTFORN+ANT_Ctb->ANTIDENTCT+ANT_Ctb->ANTORIGEM))          
   CR200LOCK("ECG",.T.)
