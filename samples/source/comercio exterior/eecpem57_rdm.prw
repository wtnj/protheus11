...
BEGIN SEQUENCE
   ...
   cData      := IncSpace(AllTrim(cExpMun)+", "+Upper(cMonth(EEC->EEC_DTCONH))+" "+AllTrim(Str(Day(EEC->EEC_DTCONH)))+", "+Str(Year(EEC->EEC_DTCONH),4),60,.f.)
   ...
   Else
      cValue := PADR(STR0006+Alltrim(EEC->EEC_PEDREF),60," ") //"OUR COMMERCIAL INVOICE NR. "
   EndIf
   ...
   For nCont := 1 To 3         
      ...
      IF HEADER_P->(RLOCK())
         ...
         HEADER_P->AVG_C01_20 := EEC->EEC_PEDREF
         // ** data da invoice
      ...
      IF lRET
         ...
         IF !Empty(Work->WKVCT)
            If SY6->Y6_TIPO = "2" // ** A VISTA.
               cVENCTO      := PADR(STR0019,25," ") //"At Sight"
            ELSE
               cVencto := IncSpace(cMonth(Work->WKVCT)+" "+AllTrim(Str(Day(Work->WKVCT)))+", "+Str(Year(Work->WKVCT),4),60,.F.)
            ENDIF
         Endif
      ENDIF
   ENDIF
