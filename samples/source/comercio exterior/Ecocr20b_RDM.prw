...
Do While .T.
   ...
   If ! FILE(cPath+("IV100.DBF"))
      E_Msg(STR0004,1) //"Arquivo IV100 näo encontrado."
      lRetLocal:=.F.
      EXIT
   EndIf
   If ! FILE(cPath+("DI100.DBF"))
      E_Msg(STR0005,1) //"Arquivo DI100 näo encontrado."
      lRetLocal:=.F.
      EXIT
   EndIf
   If ! FILE(cPath+("IVH100.DBF"))
      E_Msg(STR0006,1) //"Arquivo IVH100 näo encontrado."
      lRetLocal:=.F.
      EXIT
   EndIf
   If ! FILE(cPath+("APD100.DBF"))
      E_Msg(STR0007,1) //"Arquivo APD100 näo encontrado."
      lRetLocal:=.F.
      EXIT
   EndIf
   If lExisteECF
      If ! FILE(cPath+("ANT100.DBF"))
         E_Msg(STR0008,1) //"Arquivo ANT100 näo encontrado."
         lRetLocal:=.F.
         EXIT
      EndIf
   Endif
   ...
   IF NETERR()
      E_Msg(STR0009,1) //"Arquivo IV100 näo disponivel ..."
      lRetLocal:=.F.
      EXIT
   ENDIF
   ...
   IF NETERR()
      E_Msg(STR0012,1) //"Arquivo DI100 näo disponivel ..."
      IV_Ctb->(dbcloseAREA())
      lRetLocal:=.F.
      EXIT
   ENDIF
   ...
   IF NETERR()
      E_Msg(STR0014,1) //"Arquivo IVH100 näo disponivel ..."
      IV_Ctb->(dbcloseAREA())
      DI_Ctb->(dbcloseAREA())
      lRetLocal:=.F.
      EXIT
   ...
   IF NETERR()
      E_Msg(STR0017,1) //"Arquivo APD100 näo disponivel ..."
      IV_Ctb->(dbcloseAREA())
      DI_Ctb->(dbcloseAREA())
      IVH_Ctb->(dbcloseAREA())
      lRetLocal:=.F.
   ...
   If lExisteECF
      ...
      IF NETERR()
         E_Msg(STR0019,1) //"Arquivo ANT100 näo disponivel ..."
         IV_Ctb->(dbcloseAREA())
         DI_Ctb->(dbcloseAREA())
         IVH_Ctb->(dbcloseAREA())
         APD_Ctb->(dbcloseAREA())         
