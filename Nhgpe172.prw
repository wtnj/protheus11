...
±±ºPrograma  ³NHGPE172  ºAutor  ³Marcos R Roquitski  º Data ³  06/12/10   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Gera arquivo holerite Banco Itau 13o. Salario 2a. Parcela  º±±
±±º          ³                                                            º±±
...
If !Empty(TMP0->RA_MAT) 
	If SM0->M0_CODIGO == "FN"	// Fundicao
		cArqDep := "C:\RELATO\FN13I" + Substr(Dtos(dDataBase),5,4) + ".REM" // Deposito
	Else
		cArqDep := "C:\RELATO\NH13I" + Substr(Dtos(dDataBase),5,4) + ".REM" // Deposito
	Endif	
	cFnl    := CHR(13)+CHR(10)
...
Local _n736Fgts := _nVlCre := _nVlDeb := _nVlLiq := _nVlSal := _n701Irrf := _n735Fgts := 0
	...
	** Header de Lote 
	...
	While !TMP0->(Eof())
		...
		** SEGMENTO A 
		...
		cLin := cLin + StrZero(TMP0->RI_VALOR*100,15)
		cLin := cLin + Space(20) 
		cLin := cLin + Substr(Dtos(mv_par04),7,2)+Substr(Dtos(mv_par04),5,2)+Substr(Dtos(mv_par04),1,4)
		cLin := cLin + StrZero(TMP0->RI_VALOR*100,15)
		cLin := cLin + "HP01"
		...
		While !TMP1->(Eof())		
			...
			If TMP0->RA_MAT <> TMP1->RA_MAT
				...
				Loop
			Endif		
			If SRV->(DbSeek(xFilial("SRV") + TMP1->RI_PD)) // Verbas
						
				If TMP1->RI_PD == '739' // FGTS	
					_n739Fgts := TMP1->RI_VALOR
				Endif	
			
				If 	SRV->RV_TIPOCOD == '1' // Provento
					_nVlCre += TMP1->RI_VALOR
				Endif
				
				If SRV->RV_TIPOCOD == '2' // Desconto
					_nVlDeb += TMP1->RI_VALOR
				Endif
				
				If TMP1->RI_PD == '797' // liquido
					_nVlLiq := TMP1->RI_VALOR
					_nTotsa += TMP1->RI_VALOR					
				Endif
				...
				If TMP1->RI_PD == '705' // BASE DO I.R.R.F
					_n705Irrf := TMP1->RI_VALOR
				Endif	

				If TMP1->RI_PD == '738' // BASE DO F.G.T.S
					_n738Fgts := TMP1->RI_VALOR
				Endif	
		...
		** SEGMENTO D
		...
		cLin := cLin + TMP0->RI_CC + Space(6)
		cLin := cLin + TMP0->RI_MAT+Space(9)
		_cDescFunc := Space(30)
		...
		cLin := cLin + StrZero(_n739Fgts*100,15)
		cLin := cLin + StrZero(_nVlCre*100,15)
		cLin := cLin + StrZero(_nVlDeb*100,15)		
		cLin := cLin + StrZero(_nVlLiq*100,15)				
		cLin := cLin + StrZero(_nVlSal*100,15)		
		cLin := cLin + StrZero(_n705Irrf*100,15)
		cLin := cLin + StrZero(_n738Fgts*100,15)
		cLin := cLin + "02"
		cLin := cLin + Space(13)
		...
		fWrite(nHdl,cLin,Len(cLin)) 
		nSegt++		
		_n736Fgts := _nVlCre := _nVlDeb := _nVlLiq := _nVlSal := _n701irrf := _n735Fgts := 0
		...
		While !TMP1->(Eof())		
			...
			If SRV->(DbSeek(xFilial("SRV") + TMP1->RI_PD)) // Verbas
				If 	SRV->RV_TIPOCOD == '1' // Provento/Credito
					...
					cLin := cLin + SRV->RV_COD + ' '+Substr(SRV->RV_DESC,1,18)+SPACE(01) + Str(TMP1->RI_HORAS,7,2) + Space(05)					
					cLin := cLin + StrZero(TMP1->RI_VALOR*100,15)
					_nE  += 1
				Endif
...
cQuery += "AND RI.RI_MAT = '"+ _cMat + "'" 
cQuery += "AND RI.RI_FILIAL = '" + xFilial("SRI")+ "'" 
cQuery += "AND RA.RA_FILIAL = '" + xFilial("SRA")+ "'" 
cQuery += "ORDER BY RA.RA_MAT, RI.RI_PD"
