...
Private nEstru := 0
	...
	If !UPPER(FUNNAME())$"MATA710" // so executa o ponto de entrada a primeira vez quando é chamado pela rotina principal mata710 (MRP)
       Return cQueryB1
	Endif

    //Deixa o campo B1_MRP como não usado na rotina do MRP
...
//   MemoWrit('C:\TEMP\A710ALQ.SQL',cQuery)        
    ...
    While TEMP->(!Eof())
       ...
       While ESTRUT->(!Eof())
          ...
          If !Subs(ESTRUT->COMP,1,3)$"MOD" .And. ESTRUT->QUANT > 0  //descarta os MODs e também os retornos com esrutura negativa
             _cComp += "'"+ESTRUT->COMP+"'," 
             _lAchou := .t.      
          Endif
          ESTRUT->(DbSkip())
	   ...
	   If _lAchou // variavel de controle só executa se tiver PMP
