...
cCadastro := "Cadastro de Ficha TÈcnica de Equipamentos"
...
BrwLegenda(OemToAnsi("Ficha TÈcnica"), "Legenda", aLegenda)
...
private aTest       := {}
private aInsp       := {}
Private aTipo       := {"1=Usinagem","2=Lavadora","3=Montagem","4=Teste","5=InspeÁ„o"}             
...
If nPar == 1 .Or. nPar == 3
	...
	//Carrega os dados para visualizaÁ„o, alteraÁ„o
...
@ 25, 105 SAY OemToAnsi("CÛd. PeÁa WHB") PIXEL color CLR_HBLUE
...
@ 25, 362 SAY OemToAnsi("No. PeÁa Cliente") PIXEL 
@ 23, 405 MSGET oAp5Num VAR cZb5Ap5Num Picture "@!" WHEN .F. SIZE 080,08 OF oDlgFolder PIXEL

@ 42, 010 SAY OemToAnsi("OperaÁ„o") PIXEL color CLR_HBLUE
...
"Item","Disp. Pallet 1","Letra 1","Disp. Pallet 2","Letra 2", "Press. Fix. 1","Press. Fix. 2","Qtd PÁ/Disp";
...
"Item","Equipamento","Produto","Temperatura","ConcentraÁ„o","Tempo Lav.","Tempo Sec.","Tempo Sopro","Press„o","PeÁas/Ciclo";
...
//⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ4ø
//≥ INCLUI UM EQUIPAMENTO NA FICHA TECNICA ≥
//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ4Ÿ
...
Private nZbHPresAc  := 0
   	...
   	If nMaqPar != 2 //diferente de INCLUIR
		If fAEmpty(aMaq)
			Alert("N„o h· Ìtens selecionados!")
			Return .F.
		EndIf
    EndIf
	...
	@ 040, 175 SAY OemToAnsi("CÛd. da Maquina") OF oDlgMaq PIXEL
	...
	@ 055, 175 SAY OemToAnsi("LocalizaÁ„o da Maquina") OF oDlgMaq PIXEL
	...
	If nMaqPar == 1 .OR. nMaqPar == 3 .Or. nMaqPar == 4 //visual ou alterar
		...
		obmp1:cbmpfile  := "/SYSTEM/FT/teste.jpg" //apaga do objeto o endereÁo da imagem atual 
		obmp1:cbmpfile  := aMaq[oLbx:nAt][11]   //atribui o endereÁo da nova imagem
		obmp1:lautosize := .F.
		obmp1:lautosize := .T.      //forÁa o tamanho da imagem
		obmp1:lvisible  := .T.      //forÁa a visualizaÁ„o da imagem
	EndIf
...
//⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ4ø
//≥ INCLUI UM DISPOSITIVO NA FICHA TECNICA ≥
//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ4Ÿ
...
Private nZbGQtdPD   := 0
	...
	oTitImg2 := tSay():New(020,005,{||"Esquema de FixaÁ„o e LocalizaÁ„o"},oDlgDisp,,,,,,.T.,,)
	...
	@ 080, 175 SAY OemToAnsi("Press„o de FixaÁ„o Dispositivo 1") OF oDlgDisp PIXEL 
	...
	@ 095, 175 SAY OemToAnsi("Press„o de FixaÁ„o Dispositivo 2") OF oDlgDisp PIXEL 
	...
	@ 110, 175 SAY OemToAnsi("Quantidade de PeÁas/Dispositivo") OF oDlgDisp PIXEL 
	...
	If nDispPar == 1 .OR. nDispPar == 3 .Or. nDispPar == 4 //visual ou alterar
		...
		obmp2:cbmpfile  := "/SYSTEM/FT/teste.jpg" //apaga do objeto o endereÁo da imagem atual 
		obmp2:cbmpfile  := aDisp[oLbx2:nAt][9]   //atribui o endereÁo da nova imagem
		obmp2:lautosize := .F.
		obmp2:lautosize := .T.      //forÁa o tamanho da imagem
		obmp2:lvisible  := .T.      //forÁa a visualizaÁ„o da imagem
	EndIf
...
private cZbCNumMont := space(15) //numero da montagem da ficha tÈcnica de ferramentas
	...
	//IMAGEM FERRAMENTA 2
	oTitImg4 := tSay():New(035,160,{||"LocalizaÁ„o de Usinagem"},oDlgFer,,,,,,.T.,,)
	...
	If nFerPar == 1 .OR. nFerPar == 3 .Or. nFerPar == 4 //visual, alterar ou excluir
		...
		obmp3_1:cbmpfile  := "/SYSTEM/FT/teste.jpg" //apaga do objeto o endereÁo da imagem atual 
		obmp3_1:cbmpfile  := aFer[oLbx3:nAt][3]     //atribui o endereÁo da nova imagem
		obmp3_1:lautosize := .F.
		obmp3_1:lautosize := .T.      			  //forÁa o tamanho da imagem
		obmp3_1:lvisible  := .T.      			  //forÁa a visualizaÁ„o da imagem
		
		obmp3_2:cbmpfile  := "/SYSTEM/FT/teste.jpg" //apaga do objeto o endereÁo da imagem atual 
		obmp3_2:cbmpfile  := aFer[oLbx3:nAt][4]     //atribui o endereÁo da nova imagem
		obmp3_2:lautosize := .F.
		obmp3_2:lautosize := .T.          		  //forÁa o tamanho da imagem
		obmp3_2:lvisible  := .T.                    //forÁa a visualizaÁ„o da imagem
...
Private nZD2PpCi := 0         //peÁas por ciclo
	...
	@ 040, 175 SAY OemToAnsi("CÛd. da Maquina") OF oDlgLav PIXEL
	...
	@ 055, 175 SAY OemToAnsi("LocalizaÁ„o da Maquina") OF oDlgLav PIXEL
	...
	oSay3 := TSay():New(100,175,{||"ConcentraÁ„o do Banho"},oDlgLav,,,,,,.T.,,)
	...
	oSay11 := TSay():New(175,175,{||"PeÁas por Ciclo"},oDlgLav,,,,,,.T.,,)
	...
	If nLavPar == 1 .OR. nLavPar == 3 .Or. nLavPar == 4 //visual ou alterar
		...
		obmp4:cbmpfile  := "/SYSTEM/FT/teste.jpg" //apaga do objeto o endereÁo da imagem atual
		obmp4:cbmpfile  := aLav[oLbx:nAt][11]     //atribui o endereÁo da nova imagem
		obmp4:lautosize := .F.
		obmp4:lautosize := .T.                    //forÁa o tamanho da imagem
		obmp4:lvisible  := .T.                    //forÁa a visualizaÁ„o da imagem
	EndIf
...
Private cZD3PUHP  := Space(10)
	...
	@ 040, 175 SAY OemToAnsi("CÛd. da Maquina") OF oDlgMont PIXEL
	...
	@ 055, 175 SAY OemToAnsi("LocalizaÁ„o da Maquina") OF oDlgMont PIXEL
	...
	If nMontPar == 1 .OR. nMontPar == 3 .Or. nMontPar == 4 //visual ou alterar ou excluir
		...
		obmp5:cbmpfile  := "/SYSTEM/FT/teste.jpg" //apaga do objeto o endereÁo da imagem atual 
		obmp5:cbmpfile  := aMont[oLbx5:nAt][4]    //atribui o endereÁo da nova imagem
		obmp5:lautosize := .F.
		obmp5:lautosize := .T.                    //forÁa o tamanho da imagem
		obmp5:lvisible  := .T.                    //forÁa a visualizaÁ„o da imagem
...
Private cZD4dBem := ""        //descricao do bem
	...
	@ 040, 175 SAY OemToAnsi("CÛd. da Maquina") OF oDlgTest PIXEL
	...
	@ 055, 175 SAY OemToAnsi("LocalizaÁ„o da Maquina") OF oDlgTest PIXEL
...
//	oSay1 := TSay():New(185,05,{||},oDlgMont,,,,,,.T.,,)
	...
	If nTestPar == 1 .OR. nTestPar == 3 .Or. nTestPar == 4 //visual ou alterar
		...
		obmp6:cbmpfile  := "/SYSTEM/FT/teste.jpg" //apaga do objeto o endereÁo da imagem atual 
		obmp6:cbmpfile  := aTest[oLbx6:nAt][3]    //atribui o endereÁo da nova imagem
		obmp6:lautosize := .F.
		obmp6:lautosize := .T.                    //forÁa o tamanho da imagem
		obmp6:lvisible  := .T.                    //forÁa a visualizaÁ„o da imagem
...
*/
	...
	oTitImg7 := tSay():New(020,005,{||"Layout de InspeÁ„o"},oDlgInsp,,,,,,.T.,,)
	...
	@ 040, 175 SAY OemToAnsi("CÛd. da Maquina") OF oDlgInsp PIXEL
	...
	@ 055, 175 SAY OemToAnsi("LocalizaÁ„o da Maquina") OF oDlgInsp PIXEL
...
*/		
	...
	If nInspPar == 1 .OR. nInspPar == 3 .Or. nInspPar == 4 //visual ou alterar ou excluir
		...
		obmp7:cbmpfile  := "/SYSTEM/FT/teste.jpg" //apaga do objeto o endereÁo da imagem atual 
		obmp7:cbmpfile  := aInsp[oLbx7:nAt][3]    //atribui o endereÁo da nova imagem
		obmp7:lautosize := .F.
		obmp7:lautosize := .T.                    //forÁa o tamanho da imagem
		obmp7:lvisible  := .T.                    //forÁa a visualizaÁ„o da imagem
...
Local nItem   := 1
	...
	oLbx:SetArray( aMaq )
	oLbx:bLine := {|| {aMaq[oLbx:nAt,1],;  // ÕTEM
					   aMaq[oLbx:nAt,2],;  // M¡QUINA
    	               aMaq[oLbx:nAt,3],;  // DESCRICAO
        	           aMaq[oLbx:nAt,4],;  // POS. X
            	       aMaq[oLbx:nAt,5],;  // POS. Y
...
Static Function fMontCom()
    ...
    aadd(aHeader,{"CÛdigo"   , "ZD6_COD"  , PesqPict('ZD6', 'ZD6_COD')  , 15, 00, 'U_PCP18PRD("M->ZD6_COD")', '' , 'C', 'ZD6', ''}) 
    aadd(aHeader,{"DescriÁ„o", "B1_DESC"  , PesqPict('SB1', 'B1_DESC')  , 50, 00, '.F.'		                , '' , 'C', 'SB1', ''}) 
    aadd(aHeader,{"Qtde"     , "ZD6_QTDE" , PesqPict('ZD6', 'ZD6_QTDE'), 05, 00, '.T.'                     , '' , 'N', 'ZD6', ''}) 
	aadd(aHeader,{"PosiÁ„o"  , "ZD6_POS"  , PesqPict('ZD6', 'ZD6_POS')  , 02, 00, '.T.'         		    , '' , 'N', 'ZD6', ''})    
...
Static Function fTestItm()
    ...
    aadd(aHeader,{"CÛdigo"   , "ZD7_COD"  , PesqPict('ZD7', 'ZD7_COD')  , 15, 00, 'U_PCP18PRD("M->ZD7_COD")', '' , 'C', 'ZD7', ''}) 
    aadd(aHeader,{"DescriÁ„o", "B1_DESC"  , PesqPict('SB1', 'B1_DESC')  , 50, 00, '.F.'		                , '' , 'C', 'SB1', ''}) 
    aadd(aHeader,{"Qtde"     , "ZD7_QTDE" , PesqPict('ZD7', 'ZD7_QTDE') , 05, 00, '.T.'                     , '' , 'N', 'ZD7', ''}) 
	aadd(aHeader,{"PosiÁ„o"  , "ZD7_POS"  , PesqPict('ZD7', 'ZD7_POS')  , 02, 00, '.T.'         		    , '' , 'N', 'ZD7', ''})    
...
Static Function fInspItm()
    ...
    aadd(aHeader,{"CÛdigo"   , "ZDB_COD"  , PesqPict('ZDB', 'ZDB_COD')  , 15, 00, 'U_PCP18PRD("M->ZDB_COD")', '' , 'C', 'ZDB', ''}) 
    aadd(aHeader,{"DescriÁ„o", "B1_DESC"  , PesqPict('SB1', 'B1_DESC')  , 50, 00, '.F.'		                , '' , 'C', 'SB1', ''}) 
...
Static Function fItensMontFe()
    ...
    aadd(aHeader,{"CÛdigo"   , "ZBC_COD"  , PesqPict('ZBC', 'ZBC_COD')  , 15, 00, '.F.'/*'U_PCP18PRD()'*/, '' , 'C', 'ZBC', ''}) 
    aadd(aHeader,{"DescriÁ„o", "B1_DESC"  , PesqPict('SB1', 'B1_DESC')  , 50, 00, '.F.'		    , '' , 'C', 'SB1', ''}) 
	aadd(aHeader,{"PosiÁ„o"  , "ZBC_POS"  , PesqPict('ZBC', 'ZBC_POS')  , 02, 00, '.F.'		    , '' , 'N', 'ZBC', ''})    
...
Static Function fProcesso()
    ...
    aadd(aHeader,{"ÿ"   	     , "ZBD_DIAMET" , PesqPict('ZBD', 'ZBD_DIAMET') , 05, 02, '.T.'   , '' , 'N', 'ZBD', ''}) 
	...
	aadd(aHeader,{"vida util/pÁ" , "ZBD_VIDA"   , PesqPict('ZBD', 'ZBD_VIDA')   , 06, 00, '.T.'	  , '' , 'N', 'ZBD', ''})               
...
Static Function fLegPro()
	...
	oSay1 := tSay():New(10,05,{||"ÿ  - Di‚metro"            },oDlgLeg,,,,,,.T.,,)
	oSay1 := tSay():New(20,05,{||"vc - Velocidade de Corte" },oDlgLeg,,,,,,.T.,,)
	oSay1 := tSay():New(30,05,{||"n  - RotaÁ„o"             },oDlgLeg,,,,,,.T.,,)
	oSay1 := tSay():New(40,05,{||"z  - N˙mero de Facas"     },oDlgLeg,,,,,,.T.,,)
	oSay1 := tSay():New(50,05,{||"fz - AvanÁo por Faca"     },oDlgLeg,,,,,,.T.,,)
	oSay1 := tSay():New(60,05,{||"vf - Velocidade de AvanÁo"},oDlgLeg,,,,,,.T.,,)
...
//⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒø
//≥ VERIFICA SE O ARRAY EST¡ VAZIO ≥
//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ
...
Static Function fAtuMont()
	...
	If Empty(cCC)
		Alert("O campo do Centro de Custo no cabeÁalho da FT n„o pode estar vazio!")
		Return .F.
	EndIf
	
	If !Empty(cZbCNumMont) .and. cCC != cZbCNumMont
		Alert("N˙mero da montagem n„o contÈm o Centro de Custo da FT!")
		Return .F.
	EndIf
	...
	obmp3_1:cbmpfile  := "/SYSTEM/FT/teste.jpg" //apaga do objeto o endereÁo da imagem atual 
	obmp3_1:cbmpfile  := cStartPath + ZBJ->ZBJ_IMG01 //cStartPath + aFer[oLbx3:nAt][3]     //atribui o endereÁo da nova imagem
	obmp3_1:lautosize := .F.
	obmp3_1:lautosize := .T.      			  //forÁa o tamanho da imagem
	obmp3_1:lvisible  := .T.      			  //forÁa a visualizaÁ„o da imagem
...
//⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ4ø
//≥ GRAVA ITENS DA PROCESSO ≥                                             
//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ4Ÿ
...
Local cTipo  := ""
	...
	oImg:cbmpfile  := "/SYSTEM/FT/teste.jpg" //apaga do objeto o endereÁo da imagem atual 
	oImg:cbmpfile  := cArq01  //atribui o endereÁo da nova imagem
	oImg:lautosize := .F.
	oImg:lautosize := .T.      //forÁa o tamanho da imagem
	oImg:lvisible  := .T.      //forÁa a visualizaÁ„o da imagem
...
Local cMotRev := ""
	...
	If nPar==3 //Alterar
		...
		If ZB5->ZB5_STATUS=="A"
			If !(MsgYesNo("Esta alteraÁ„o ir· gerar uma nova revis„o. Esta revis„o ficar· pendente. Deseja continuar?"))
				Return
			EndIf
	                  
			oDlgRev  := MsDialog():New(0,0,210,400,"Ficha TÈcnica",,,,,CLR_BLACK,CLR_WHITE,,,.T.)
		...
		If lRev
			...
			IF ZB5->(DBSEEK(XFILIAL("ZB5")+cZb5NumFt))
				WHILE ZB5->(!EOF()) .AND. ZB5->ZB5_NUM == cZb5NumFt
				    ...
				    cZb5Rev  := StrZero(Val(ZB5->ZB5_REV)+1,3) //Pega o prÛximo n˙mero da revis„o
					dZB5DtElab   := ZB5->ZB5_DTELAB
				    cZb5Elab := ZB5->ZB5_ELABOR
				    cResRev   := Alltrim(UPPER(cUserName))
				    
				    RecLock("ZB5",.F.)
				    	ZB5->ZB5_ULTREV := 'N'//Marca que n„o È mais a ˙ltima revis„o
				    	ZB5->ZB5_STATUS := 'O'
				    MsUnLock("ZB5")
	    ...
	    For _x := 1 to Len(aMaq)
			...
			If !empty(aMaq[_x][11]) .and. !(cStartPath+cImgTmp$aMaq[_x][11])
				If __CopyFile(aMaq[_x][11],cStartPath+cImgTmp)
					aMaq[_x][11] := cImgTmp
				Else
					Alert("ImpossÌvel copiar Imagem do Equipamento "+aMaq[_x][1]+"!")
					aMaq[_x][11] := ""
				EndIf
			Else
	    ...
	    For _x := 1 to Len(aDisp)
			...
			If !empty(aDisp[_x][9]) .AND. !(cStartPath+cImgTmp$aDisp[_x][9])
				If __CopyFile(aDisp[_x][9],cStartPath + cImgTmp)
					aDisp[_x][9] := cImgTmp
				Else
					Alert("ImpossÌvel copiar Imagem do Dispositivo "+aDisp[_x][1]+"!")
					aDisp[_x][9] := ""
				EndIf
			Else
	    ...
	    For _x := 1 to Len(aFer)
			...
			If !Empty(aFer[_x][4]) .AND. !(cStartPath+cImgTmp$aFer[_x][4])
				If __CopyFile(aFer[_x][4],cStartPath + cImgTmp)
					aFer[_x][4] := cImgTmp
				Else
					Alert("ImpossÌvel copiar Imagem 2 da Ferramenta "+aFer[_x][1]+"!")
					aFer[_x][4] := ""
				EndIf
			Else
		...
		For _x:=1 to Len(aLav)
			...
			If !empty(aLav[_x][11]) .and. !(cStartPath+cImgTmp$aLav[_x][11])
				If __CopyFile(aLav[_x][11],cStartPath+cImgTmp)
					aLav[_x][11] := cImgTmp
				Else
					Alert("ImpossÌvel copiar Imagem da Lavadora "+aLav[_x][1]+"!")
					aLav[_x][11] := ""
				EndIf
			Else
        ...
        For _x:=1 to Len(aMont)
			...
			If !empty(aMont[_x][4]) .and. !(cStartPath+cImgTmp$aMont[_x][4])
				If __CopyFile(aMont[_x][4],cStartPath+cImgTmp)
					aMont[_x][4] := cImgTmp
				Else
					Alert("ImpossÌvel copiar Imagem da Montgem "+aMont[_x][1]+"!")
					aMont[_x][4] := ""
				EndIf
			Else
		...
		For _x:=1 to Len(aTest)
			...
			If !empty(aTest[_x][3]) .and. !(cStartPath+cImgTmp$aTest[_x][3])
				If __CopyFile(aTest[_x][3],cStartPath+cImgTmp)
					aTest[_x][3] := cImgTmp
				Else
					Alert("ImpossÌvel copiar Imagem do Teste "+aTest[_x][1]+"!")
					aTest[_x][3] := ""
				EndIf
			Else
			...
			//Percorre o array aItmTest que È o aTest[_x][4]
			aItmTest := aTest[_x][4]
			...
			//Percorre o array aParTest, que È o aTest[_x][5]
			aParTest := aTest[_x][5]
		...
		For _x:=1 to Len(aInsp)
			...
			If !empty(aInsp[_x][3]) .and. !(cStartPath+cImgTmp$aInsp[_x][3])
				If __CopyFile(aInsp[_x][3],cStartPath+cImgTmp)
					aInsp[_x][3] := cImgTmp
				Else
					Alert("ImpossÌvel copiar Imagem da InspeÁ„o "+aInsp[_x][1]+"!")
					aInsp[_x][3] := ""
				EndIf
			Else
	...
	**********/
	If nPar == 3
		...
		If ZB5->ZB5_STATUS=="A"
			If !(MsgYesNo("Esta alteraÁ„o tornar· a Ficha TÈcnica pendente. Deseja continuar?"))
				Return
			EndIf
		EndIf
		...
		//VERIFICA SE N√O EXISTEM ALGUM ITEM NA TABELA QUE TENHA SIDO EXCLUÕDO NO ARRAY AMAQ	
		ZBH->(DbSetOrder(1)) //filial + numft + rev + item
		...
		//VERIFICA SE N√O EXISTEM ALGUM ITEM NA TABELA QUE TENHA SIDO EXCLUÕDO NO ARRAY ALAV
        ...
        For _x:=1 to Len(aMont)
			...
			If !Empty(aMont[_x][4]) .AND. !(cImgTmp$aMont[_x][4])
				If __CopyFile(aMont[_x][4],cStartPath + cImgTmp)
					aMont[_x][4] := cImgTmp
				Else
					Alert("ImpossÌvel copiar Imagem da Montagem "+aMont[_x][1]+"!")
					aMont[_x][4] := ""
				EndIf
			EndIf
        ...
        For _x:=1 to Len(aTest)
			...
			If !Empty(aTest[_x][3]) .AND. !(cImgTmp$aTest[_x][3])
				If __CopyFile(aTest[_x][3],cStartPath + cImgTmp)
					aTest[_x][3] := cImgTmp
				Else
					Alert("ImpossÌvel copiar Imagem da Montagem "+aTest[_x][1]+"!")
					aTest[_x][3] := ""
				EndIf
			EndIf
		...
		//VERIFICA SE N√O EXISTEM ALGUM ITEM NA TABELA QUE TENHA SIDO EXCLUÕDO NO ARRAY AINSP
...
Static Function fValidaFT()
	...
	If nPar==3//alterar
		If ZB5->ZB5_ULTREV=="N"
			Alert("N„o È possÌvel alterar! Documento n„o est· em sua ˙ltima revis„o.")
			Return .F.
		EndIf
	EndIf
	...
	//codigo da peca
	If Empty(cZb5Peca)
		MsgBox("CÛdigo da peÁa em branco!"+_CRLF+;
				"Digite o cÛdigo da peÁa.","PeÁa em branco","ALERT")
		Return .F.
	EndIf 
	//numero da ft
	IF Empty(cZb5NumFT)
		MsgBox("CÛdigo da Ficha TÈcnica em branco!"+_CRLF+;
				"Digite o cÛdigo da peÁa e a OperaÁ„o.","N˙mero da FT em branco","ALERT")
		Return .F.
	EndIf
	//operacao
	If Empty(cZb5oP)
		MsgBox("OperaÁ„o em branco!"+_CRLF+;
				"Digite a OperaÁ„o.","OperaÁ„o em branco","ALERT")
		Return .F.
	EndIf
...
Local cNewFtNum := ""
	...
	If !Empty(cZb5Peca) .AND. !Empty(cZb5Op)
		...
		If DBSEEK(XFILIAL("ZB5")+cNewFtNum)
			Alert("FT j· existe para esta PeÁa x OperaÁ„o!"+_CRLF+;
				  "Informe outra PeÁa ou OperaÁ„o.")
			Return .F.
		EndIf
	EndIf
...
Static Function fBem(cBem,nTab)
	...
	Else
		Alert("M·quina n„o foi encontrada!"+_CRLF+;
			  "Informe um cÛdigo de m·quina existente.")
		Return .F.
	Endif
...
Private aCols   := {}
	...
	oDlgApr := MsDialog():New(0,0,200,400,"Ficha TÈcnica",,,,,CLR_BLACK,CLR_WHITE,,,.T.)
...
Private cMotRev := ""
	...
	If ZB5->ZB5_STATUS != "P"
		Alert("A Ficha TÈcnica deve estar pendente para aprovar!")
		Return
	EndIf
	...
	oDlg  := MsDialog():New(0,0,210,400,"Ficha TÈcnica",,,,,CLR_BLACK,CLR_WHITE,,,.T.)
...
±±ÃÕÕÕÕÕÕÕÕÕÕÿÕÕÕÕÕÕÕÕÕÕ ÕÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕ ÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕπ±±
±±∫Desc.     ≥ Cria uma p·gina HTML para exibir a Ficha TÈcnica no IE     ∫±±
±±∫          ≥                                                            ∫±±
±±ÃÕÕÕÕÕÕÕÕÕÕÿÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕπ±±
±±∫Uso       ≥ AP                                                         ∫±±
±±»ÕÕÕÕÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕº±±
...
Local cArqHtml := AllTrim(StrTran(ZB5->ZB5_NUM,"/","_"))+".html"
	...
	cPAG += '<title>Ficha TÈcnica</title>'

	//Aqui È montada uma p·gina central onde contÈm os links para as folhas de ferramenta
	cPAG += '<body>'
...
Local cZD8Item  := ""
	...
	cHTML += '<title>Ficha TÈcnica</title>'
...
//	cHTML += '  height:50%;'
	...
	cHTML += '      <div class="titulo">DescriÁ„o da OperaÁ„o:</div>'
	...
	ElseIf nPar == 3 //ferramenta
		...
		cHTML += '	        <td align="center">Vida ⁄til / PÁ</td>'
		cHTML += '	      </tr>'
