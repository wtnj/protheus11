...
If cTipo == "1"
		...
		Else
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD2
						...
						D2_ORIGLAN,D2_VALCF3,D2_VALIPI,D2_VALACRS,D2_PICM %Exp:cField% 
						FROM %Table:SD2% SD2
						WHERE
	...
	//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
            		      ...
            		      If (cAliasSD2)->D2_TP == "PA" .Or. (cAliasSD2)->D2_TES == "578"//Para tratar a TAG InfAdProd
            		         ...
            		         Else //Itesapar
            		            ...
            		            If (cAliasSD2)->D2_CLIENTE$ "900077" //Cliente Bosch 
            		               cInfAdic := "Cod. Ped.: "+Alltrim(_cPC) +" - Fabricacao: " +StrZero(Day(ddatabase),2)+'/'+StrZero(Month(ddatabase),2)+'/'+StrZero(Year(ddatabase),4) + " - Cod. Cliente: 00010"+ Alltrim(_cCodcli)
            		            Else
            		               cInfAdic := "Cod. Ped.: "+Alltrim(_cPC) +" - Fabricacao: " +StrZero(Day(ddatabase),2)+'/'+StrZero(Month(ddatabase),2)+'/'+StrZero(Year(ddatabase),4) + " - Cod. Cliente: 00001"+ Alltrim(_cCodcli)            		            
            		            Endif   
	            		     Endif
            		      Endif   
 	                  Else
				...
				//-- WHB FIM 
		            ...
		            /* Tratamento da Clausula DÈcima do Ajuste SINIEF 19/2012 - CHAMADO TGTGNB */
					If (SF4->(FieldPos("F4_CONSUMO")) > 0 .And. SF4->F4_CONSUMO == "N") .And. (cOrigem $"1-2-3" .And. cCSTrib $ "00-10-20-70-90") .And. ((cAliasSD2)->D2_PICM == 4)
						lFCI	:= .T.
						SPEDRastro2((cAliasSD2)->D2_NUMLOTE,(cAliasSD2)->D2_LOTECTL,cCodProd,@nBaseIcm,@nValICM,lFCI)
						If nBaseIcm > 0
							nValParImp := (cAliasSD2)->D2_QUANT*nBaseIcm   //Valor da Parcela Importada (valor da importaÁ„o correspondete ao valor da base de calculo de icms na Op de importaÁ„o)
							nValTotOpe := (cAliasSD2)->D2_TOTAL+(cAliasSD2)->D2_VALIPI// Valor Total da OperaÁ„o de SaÌda Interestadual (incluindo tributos incidentes nas operaÁıes proprias do remetente)
							nContImp   := (nValParImp/nValTotOpe)*100  // Conte˙do da ImportaÁ„o (% entre o valor da parcela importada e o valor total da operaÁ„o)
							
							cInfAdic  += "Resolucao do Senado Federal n∫ 13/12, Valor da Parcela Importada R$ "+ ConvType(nValParImp, 11,2)+ ", Conteudo de Importacao " + ConvType(nContImp, 11,2) + "% , Valor da Importacao R$ " + ConvType(nBaseIcm, 11,2)
						EndIf
					EndIf	            
						...
						Do Case
							...
							Case AllTrim(CD2->CD2_IMP) == "IPI"
								If !lConsig
									...
									EndIf
									If !lIpiDev .And. !(Alltrim((cAliasSD2)->D2_CF) $ cMVCFOPREM) .OR. lIpiBenef
					...
					If ((cAliasSD2)->D2_TIPO == "D" .and. !lIpiDev)  .Or. lConsig .Or. (Alltrim((cAliasSD2)->D2_CF) $ cMVCFOPREM ) .OR. ((cAliasSD2)->D2_TIPO == "B" .and. lIpiBenef)
				...
				If !Empty((cAliasSD1)->D1_NFORI)
					...
					If MsSeek(xFilial("SD1")+cSeekD1)
						...
						Else
							...
							MsSeek(xFilial("SA2")+SD1->D1_FORNECE+SD1->D1_LOJA)
						EndIf
				        lRural := AllTrim(SF1->F1_ESPECIE) $ "NFP-NFA"
					...
					If !(cAliasSD1)->D1_TIPO $ "DBN" .Or. lRural
						...
						//≥    da tabela SA2.                                                                            ≥
						//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ
						If AllTrim(SF1->F1_ESPECIE)$ "NFP-NFA" .and. lSeekOk
							//para nota de entrada tipo devolucao o emitente eh o cliente ou o sigamat no caso de formulario proprio=sim
							If SD1->D1_TIPO$"DB"
								aadd(aNfVincRur,{SD1->D1_EMISSAO,SD1->D1_SERIE,SD1->D1_DOC,SF1->F1_ESPECIE,;
									IIF(SD1->D1_FORMUL=="S",SM0->M0_CGC,SA1->A1_CGC),; 
...
If !Empty(aNota)
	...
	For nX := 1 To Len(aProd)
		If nLenaIpi > 0/* WHB INICIO */ .And.  nX <= nLenaIpi //-- WHB FIM
			...
			EndIf
		EndIf
		cString += NfeItem(aProd[nX],aICMS[nX],aICMSST[nX],aIPI[nX],aPIS[nX],aPISST[nX],aCOFINS[nX],aCOFINSST[nX],aISSQN[nX],aCST[nX],aMed[nX],aArma[nX],aveicProd[nX],aDI[nX],aAdi[nX],aExp[nX],aPisAlqZ[nX],aCofAlqZ[nX],aAnfI[nX],cTipo,cVerAmb, aComb[Nx],@cMensFis,aCsosn[Nx],aPedCom[nX],aNota[5],aICMSZFM[nX],aDest,cIpiCst)
	Next nX
	...
	cString += NfeInfAd(cMensCli,cMensFis,aPedido,aExp,cAnfavea,aMotivoCont,aNota,aNfVinc,aProd,aDI,aNfVincRur,aRetido,cNfRefcup,cSerRefcupc,cTipo,aTransp)
	cString += "</infNFe>"
EndIf
...
Static Function NfeItem(aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ,aAnfI,cTipo,cVerAmb,aComb,cMensFis,cCsosn,aPedCom,cF2Tipo,aICMSZFM,aDest,cIpiCst)
...
//Local lSimpNac   	:= SuperGetMV("MV_CODREG")== "1" .Or. SuperGetMV("MV_CODREG")== "2" 
Local lUnTribCom	:= GetNewPar("MV_VTRICOM",.F.) /* Par‚metro para informar o valor unit·rio comercial e valor unit·rio tribut·vel nas
													informaÁıes complementares do DANFE (quando vuncom e vuntrib forem diferentes)*/
...
MsSeek(xFilial("SF4")+aProd[27]) 
if SF4->(FieldPos("F4_MSGLT"))>0
	if SF4->F4_MSGLT $ "1" .And. !empty(aProd[20])
		if !empty(aProd[25])
			cString += '<infadprod>'+cLoteProd+" - "+ConvType(aProd[25],500)+ cMensDeson+'</infadprod>'
		else
			cString += '<infadprod>'+ cLoteProd + cMensDeson+'</infadprod>'
		endif	
	elseif !SF4->F4_MSGLT $ "2"
		cString += '<infadprod>'+ConvType(aProd[25],500)+cMensDeson+'</infadprod>'  
	endif 
else
 	cString += '<infadprod>'+ConvType(aProd[25],500)+cMensDeson+'</infadprod>'
endif
...
Static Function NfeInfAd(cMsgCli,cMsgFis,aPedido,aExp,cAnfavea,aMotivoCont,aNfSa,aNfVinc,aProd,aDI,aNfVincRur,aRet,cNfRefcup,cSerRefcup,cTipo,aTransp)
...
If Len(cMsgCli)>0
			...
			IF (SM0->M0_ESTENT) $ "SP" .AND. cTipo=='0'
				SFT->(dbSetOrder(6))
...
If SM0->M0_CODIGO$"IT" .And. cTipo=="1" //somente nota de saida p itesapar
	...
	cString += "CHEGADA: " + StrZero(Day(ddatabase)+1,2)+'/'+StrZero(Month(ddatabase),2)+'/'+StrZero(Year(ddatabase),4)+" 16:00 " 
    If Len(aTransp[01])==14
