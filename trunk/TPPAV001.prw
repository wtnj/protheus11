...
±±ºPrograma  ³TPPAV001  º Autor ³ HANDERSON DUARTE   º Data ³  05/01/09   º±±
...
User Function TPPAV001 (cMatr,cRotina)
Local lRet	:=	.F.
	...
	Do Case
		Case cRotina	==  "ZC9"   //Indica quais os usuario (QAA) poderao aprovar o processo de Historicos do Processo
			...
			If QAA->(DBSeek(xFilial(cString)+cMatr))
				...
				Else
					lRet	:=	.F.
					Aviso("TPPAV001->Atencao!","O usuário não tem permissão para assinar o documento. Verifique o parâmentro WHB_APRHIS." ,{"OK"},2)						
				EndIf	
			Else
				lRet	:=	.F.	
				Aviso("TPPAV001->Atencao!","O usuário não tem permissão para assinar o documento. Verifique o parâmentro WHB_APRHIS." ,{"OK"},2)										
		...
		Case cRotina	==  "WHB_METRO"   //Indica quais são os códigos dos usuários da metrologia que poderão aprovar o documento.
			...
			If QAA->(DBSeek(xFilial(cString)+cMatr))
				...
				Else
					lRet	:=	.F.
					Aviso("TPPAV001->Atencao!","O usuário não tem permissão para assinar o documento. Verifique o parâmentro WHB_METRO." ,{"OK"},2)						
				EndIf	
			Else
				lRet	:=	.F.	
				Aviso("TPPAV001->Atencao!","O usuário não tem permissão para assinar o documento. Verifique o parâmentro WHB_METRO." ,{"OK"},2)										
		...
		Case cRotina	==  "WHB_ENGEN"   //Indica quais são os códigos dos usuários da engenharia que poderão aprovar o documento.
			...
			If QAA->(DBSeek(xFilial(cString)+cMatr))
				...
				Else
					lRet	:=	.F.
					Aviso("TPPAV001->Atencao!","O usuário não tem permissão para assinar o documento. Verifique o parâmentro WHB_ENGEN." ,{"OK"},2)						
				EndIf	
			Else
				lRet	:=	.F.	
				Aviso("TPPAV001->Atencao!","O usuário não tem permissão para assinar o documento. Verifique o parâmentro WHB_ENGEN." ,{"OK"},2)										
		...
		OtherWise
			lRet	:=	.F.
			Aviso("TPPAV001->Atencao!","Rotina sem de controle de alçada",{"OK"},2)					
	EndCase	
...
Else
	...
	If QAA->(DBSeek(xFilial(cString)+cMatr))
		...
		Else
			lRet	:=	.F.
			Aviso("TPPAV001->Atencao!","O usuário não tem permissão para assinar o documento. Verifique os parâmentros WHB_CARGO e WHB_DPTO." ,{"OK"},2)						
		EndIf	
	Else
		lRet	:=	.F.	
		Aviso("TPPAV001->Atencao!","O usuário não tem permissão para assinar o documento. Verifique os parâmentros WHB_CARGO e WHB_DPTO." ,{"OK"},2)										
