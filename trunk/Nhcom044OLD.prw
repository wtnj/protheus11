...
Aadd(aFields,{"DT_VALORP"     ,"N",OemToAnsi("Total PA"),"@E 999,999,999.99"})
	...
	bEnchoice := {||EnchoiceBar(oDialog,{||oDialog:End()},{||oDialog:End()},,aButtons)}
...
aRotina := { {"Anexo 1"      ,'U_fAnexodet(1)', 0 , 1 },;
             ...
             {"Imprime"      ,'U_fImpPend()', 0 , 1 },;
             {"Filtrar"      ,'U_fFilPend()', 0 , 1 }} 

cDelFunc  := ".T."
cMarca    := getmark()
             
MarkBrow("XDET","","",afields,,)
*/
...
SZU->(DbSetOrder(1))
SZU->( DbGotop())
SZU->(DbSeek(xFilial("SZU")+UPPER(ALLTRIM(cUserName))))

ProcRegua(0)

While !SZU->(Eof()) .AND. Alltrim(SZU->ZU_LOGIN) == Alltrim(cUserName)
	...
	While !SZU->(Eof()) .And. SZU->ZU_NUM+SZU->ZU_ITEM == _cNumSc
		...
		If (SZU->ZU_STATUS $ "B/C" .And. Alltrim(SZU->ZU_LOGIN) <> Alltrim(cUserName) )
			_cOk := "N"
			Exit
		Else
	...
	If SC1->(Found()) .AND. Empty(SC1->C1_RESIDUO) .AND. _cOk == 'S'	
		ApTitulo1()
	Endif
	SZU->(DbSkip())
Enddo
...
DbSelectarea("SZV")
SZV->(DbSetOrder(2))
SZV->(DbSeek(xFilial("SZV")+upper(alltrim(cUserName))))
While !SZV->(Eof())
	If (DATE() >= SZV->ZV_DATDE .AND. DATE() <= SZV->ZV_DATAT)
		If Alltrim(SZV->ZV_LOGDE) == Alltrim(cUserName)
...
Local _nUltPrc := 0
	...
	XDET->DT_STATUS    := SZU->ZU_STATUS			
	XDET->DT_OBS       := SC1->C1_OBS     	
	XDET->DT_ULTAPR    := _dUltApr
