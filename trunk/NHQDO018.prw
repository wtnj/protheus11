...
User Function NhQdo018(_pInclui)
	...
	Elseif SM0->M0_CODIGO == 'FN' // WHB FUNDICAO
		...
		If Alltrim(M->QDH_CODTP) $  'CAJ/PCJ/DCJ' //INSERIDO O PCJ E DCJ A PEDIDO DO FELIPE PERES (FORJARIA)
//			Alterado de 'Substr(M->QDH_PRODUT,4,2)' PARA 'AllTrim(M->QDH_AREA)' em 22/07/2013, a pedido do Felipeob (OS 053400)
		...
		Elseif Alltrim(M->QDH_CODTP) == 'TB' 
		   ...
		   nTipoCod := 4 
		/***** Incluido o 'CAU' a pedido da Maria Andrea Sgoda (21/10/2013)*****/
		Elseif Alltrim(M->QDH_CODTP) $  'DFJ/FIU/LFU/MTU/PCU/PKU/LFV/PCV/PKV/MMU/DCA/PKA/CAU/PCP/PKP/MTP/CAP'//RETIRADO O PKJ POR SOLICITAÇÃO DA FABIANA. OS 47477 - Retirado o DCJ e PCJ a pedido do Felipe Peres (Forjaria)
		...
		Elseif Substring(Alltrim(M->QDH_CODTP),1,3) == 'FEU' .OR. Alltrim(M->QDH_CODTP) == 'FEP'
		   cCodDoc  := Substr(Alltrim(M->QDH_CODTP),1,3) + Substr(M->QDH_PRODUT,4,2) + "." 
		   nTipoCod := 2 
		...
		Elseif Alltrim(M->QDH_CODTP) == 'TBA' // TB01.001.001 
		   cCodDoc  := Substr(Alltrim(M->QDH_CODTP),1,3) + Alltrim(M->QDH_AREA) + "." + StrZero(Val(Alltrim(M->QDH_SUBPRO)),3) 
		   nTipoCod := 12 
		...
		Elseif Alltrim(M->QDH_CODTP) $ 'CAF/PCF/CAA/PVA/FPVA/PCA/DG'
			cCodDoc := Alltrim(M->QDH_CODTP) + Substr(M->QDH_PRODUT,1,3) + "."
			nTipoCod := 8
...
/*****************************************************************************/
/************ INCLUÍDO POR GUILHERME CAMARGO, A PEDIDO DA FABIANA ************/
/**********************    OS 47477 (11/03/2013)   ***************************/
/*****************************************************************************/
		Elseif Alltrim(M->QDH_CODTP) == 'PKJ'
			cCodDoc  := Alltrim(M->QDH_CODTP) + Alltrim(M->QDH_AREA) + "." 
			nTipoCod := 5
