...
±±ºPrograma  ³NHGPE243  ºAutor  ³Marcos R. Roquitski º Data ³  10/05/2013 º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Recadastramento de vale transporte.                         º±±
±±º          ³                                                            º±±
...
Local i := 0
	...
	@ 1, 25 Psay "SOLICITAÇÃO DE VALE TRANSPORTE"
	...
	@ Prow()+2, 00 Psay "(  ) Opto pela utilização do Vale-Transporte"
	@ Prow()+2, 00 Psay "(  ) Nao opto pela utilização do Vale-Transporte" 


	@ Prow()+3, 00 Psay "Declaro que as linhas abaixo discriminadas são as mais adequadas ao trajeto"

	@ Prow()+2, 00 Psay "residencia/trabalho e vice versa:"

	@ Prow()+3, 00 Psay " Linha Descrição                            Quantidade"	
	@ Prow()+1, 00 Psay ""	
	...
	@ Prow()+2, 00 Psay "Autorizo  a  empresa  a  descontar  até  o  limite de 6% do meu salário mensal," 
	@ Prow()+1, 00 Psay "conforme  legislação  vigente  LEI Nº 7.418,  destinado a cobrir o fornecimento"
	@ Prow()+1, 00 Psay "de vales transporte por mim utilizados."

	@ Prow()+2, 00 Psay "Declaro  que   as   linhas   acima  discriminadas  são   verdadeiras  e atuais." 

	@ Prow()+2, 00 Psay "A declaração  falsa  ou  o  uso indevido do benefício caracteriza a rescisão do"
	@ Prow()+1, 00 Psay "contrato  individual  de trabalho por justa causa, ato de improbidade, conforme"
	@ Prow()+1, 00 Psay "art., 482 da CLT."
                                                   
	@ Prow()+2, 00 Psay "Autorizo,  também,  que na  hipótese de existirem créditos em meu cartão quando" 
	@ Prow()+1, 00 Psay "da consulta  realizada  pela  empregadora,  esta  creditará apenas a quantidade"
	@ Prow()+1, 00 Psay "faltante  para  se  completar os vales-transporte que utilizo a cada mês. E nos"
	@ Prow()+1, 00 Psay "dias  em  que  me  deslocar ao trabalho por outro meio de transporte não ônibus"
	@ Prow()+1, 00 Psay "coletivo, autorizo a empresa  a deduzir no mês subsequente a quantidade de vale"
	@ Prow()+1, 00 Psay "transporte que não foram utilizados no mês.

	@ Prow()+2, 00 Psay "Em  caso  de  perda,  roubo   ou   extravio  do  meu  cartão   vale-transporte,"
	@ Prow()+1, 00 Psay "comprometo-me  a  fazer  solicitação  de  um  novo  cartão  e,  posteriormente,"
