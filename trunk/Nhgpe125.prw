...
±±ºPrograma  ³NHGPE125  ºAutor  ³Marcos R Roquitski  º Data ³  07/12/07   º±±
...
If !Empty(TMP->ZRA_MAT)
	If SM0->M0_CODIGO == "FN"	// Fundicao
		cArqDep := "FNBD" + Substr(Dtos(dDataBase),5,4) + ".REM" // Deposito
	Else
		cArqDep := "NHBD" + Substr(Dtos(dDataBase),5,4) + ".REM" // Deposito
	Endif	
	cFnl    := CHR(13)+CHR(10)
...
If !Empty(TMD->ZRA_MAT)
	If SM0->M0_CODIGO == "FN"	
		cArqDoc := "FNBC" + Substr(Dtos(dDataBase),5,4) + ".REM" // DOC
	Else
		cArqDoc := "NHBC" + Substr(Dtos(dDataBase),5,4) + ".REM" // DOC
	Endif	
	cFnl    := CHR(13)+CHR(10)
...
Local _nTotsa := 0
	...
	** SEGMENTO C
	...
	cLin := cLin + Space(01)
	cLin := cLin + "2"
	cLin := cLin + _cNpj  // Subst(SM0->M0_CGC,1,14)
...
While !TMP->(Eof())
	...
	If TMP->ZRA_BANCO == mv_par01
		...
		** SEGMENTO A 
		...
		cLin := cLin + StrZero(TMP->ZRC_VALOR*100,15) 
		cLin := cLin + Space(20) 
		cLin := cLin + "00000000" 
		cLin := cLin + "000000000000000"  
...
Local _nTotsa := 0
	...
	** Header
	...
	cLin := cLin + Space(9)
	cLin := cLin + "2"
	cLin := cLin + _cNpj //Substr(SM0->M0_CGC,1,14)
	cLin := cLin + Space(20)
	cLin := cLin + StrZero(val(substr(mv_par02,1,4)),5) // Agencia
	cLin := cLin + Substr(mv_par02,5,1) // digito agencia
	cLin := cLin + fGeracc()
...
While !TMT->(Eof())
	...
	** SEGMENTO A 
	...
	cLin := cLin + "BRL" 
	cLin := cLin + "000000000000000" 
	cLin := cLin + StrZero(TMT->ZRC_VALOR*100,15) 
	cLin := cLin + Space(20) 
	cLin := cLin + "00000000" 
	cLin := cLin + "000000000000000"  
...
While !TMD->(Eof())
	...
	** SEGMENTO A 
	...
	cLin := cLin + "BRL" 
	cLin := cLin + "000000000000000" 
	cLin := cLin + StrZero(TMD->ZRC_VALOR*100,15) 
	cLin := cLin + Space(20) 
	cLin := cLin + "00000000" 
	cLin := cLin + "000000000000000"  
