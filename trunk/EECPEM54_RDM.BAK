...
//AWR - 23/04/2009 \/  \/
IF WorkId->EEA_COD = "92"
	cNome_Exp := SA2->A2_NOME
	cMunData  := ALLTRIM(SA2->A2_MUN)+", "+Upper( cMonth(dDataBase) )+" "+StrZero(Day(dDataBase),2)+", "+Str(Year(dDataBase),4)+"."+SPACE(15)
	
	
	//Nome do Contato sequencia 001
	//Contato x Cargo
	If !Alltrim(Upper(cusername))$'EDUARDOM'
		cEXP_CONTATO := EECCONTATO(CD_SA2,cCod,cLoja,"1",1) //LUIZ ALEXANDRE FRIEDRICH
		cEXP_CARGO   := EECCONTATO(CD_SA2,cCod,cLoja,"1",2) //EXPORT MANAGER
	ElseIf Alltrim(Upper(cusername))$'EDUARDOM'
		//Nome do Contato sequencia 002
		//Contato x Cargo
		cEXP_CONTATO := EECCONTATO(CD_SA2,cCod,cLoja,"2",1) //EDUARDO MARIANO
		cEXP_CARGO   := EECCONTATO(CD_SA2,cCod,cLoja,"2",2) //EXPORT ANALYST
	Endif
...
IF WorkId->EEA_COD = "92"
	// RODAPE
	HEADER_P->AVG_C26_60 := AllTrim(cNome_Exp)
	HEADER_P->AVG_C27_60 := AllTrim(cMunData)
	HEADER_P->AVG_C28_60 := AllTrim(cEXP_CONTATO)
	HEADER_P->AVG_C29_60 := AllTrim(cEXP_CARGO)
ENDIF
//AWR - 23/04/2009 /\ /\
	...
	If(!Empty(SA2->A2_BAIRRO)," - "+AllTrim(SA2->A2_BAIRRO),"") +;
		...
		If(!Empty(SA2->A2_FAX),"  FAX.: "+AllTrim(SA2->A2_FAX),"")
			...
			*-------------------------*
			...
			While EE9->(!Eof() .And. EE9_FILIAL == xFilial("EE9")) .And.;
				...
				IF ! EE2->(Dbseek(xFilial("EE2")+"8"+"*"+EEC->EEC_IDIOMA+EE9->EE9_UNIDAD))
					MsgStop("Unidade de medida "+EE9->EE9_UNIDAD+" nào cadastrada em "+EEC->EEC_IDIOMA,"Aviso") //"Unidade de medida "###" nào cadastrada em "###"Aviso"
				Endif
				...
				Else
					...
					Elseif At("PALLET",EE5->EE5_DESC) > 0
				...
				For i := 2 To MlCount(cMemo,TAMDESC,3)
					IF !EMPTY(MemoLine(cMemo,TAMDESC,i))
						...
						If lCodProd
							...
							If SB1->(Found())
								DETAIL_P->AVG_C01_20 := AllTrim(SB1->B1_CODAP5)
							Else
								DETAIL_P->AVG_C01_20 := EE9->EE9_COD_I//" "
							Endif
							lCodProd := .f.
						Endif
					ENDIF
			...
			*-------------------------*
			...
			Local nPosLin:=10, nCol:=50
			
			Begin Sequence
			
			
			DEFINE MSDIALOG oDlg TITLE "Dados do rodape" FROM 0,0 TO 15,45 OF oMainWnd
			
			
			@ nPosLin,002 SAY "Nome" SIZE 20,10 PIXEL
			@ nPosLin,nCol GET cNome_Exp SIZE 120,08 PIXEL
			nPosLin+=15
			
			@ nPosLin,002 SAY "Municipio, Data" SIZE 50,10 PIXEL
			@ nPosLin,nCol GET cMunData SIZE 120,08 PIXEL
			nPosLin+=15
			
			@ nPosLin,002 SAY "Assinante" SIZE 25,10 PIXEL
			@ nPosLin,nCol GET cEXP_CONTATO SIZE 120,08 PIXEL
			nPosLin+=15
			
			@ nPosLin,002 SAY "Cargo" SIZE 20,10 PIXEL
			@ nPosLin,nCol GET cEXP_CARGO SIZE 120,08 PIXEL
			nPosLin+=25
			
			DEFINE SBUTTON oBtnOk     FROM nPosLin,058 TYPE 1 ACTION Eval(bOk) ENABLE OF oDlg
			DEFINE SBUTTON oBtnCancel FROM nPosLin,088 TYPE 2 ACTION Eval(bCancel) ENABLE OF oDlg
