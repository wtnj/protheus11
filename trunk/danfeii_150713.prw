...
#DEFINE MAXITEMC   030 //-- WHB ALTERADO DE 018 PARA 030              // Máxima de caracteres por linha de produtos/serviços
...
#DEFINE MAXVALORC  012 //-- WHB ALTERADO DE 008 PARA 012              // Máximo de caracteres por linha de valores numéricos
				...
				If !Empty(aXML[nX][2]) .And. Empty((cAliasSFT)->FT_DTCANC)
					...
					If ( !Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5,7" )
						...
						If aNotas[nX][02]=="E" .And. MV_PAR04==1 .And. (oNfe:_NFE:_INFNFE:_IDE:_TPNF:TEXT=="0")
							dbSelectArea("SF1")
							dbSetOrder(1)
							If MsSeek(xFilial("SF1")+aNotas[nX][05]+aNotas[nX][04]) .And. SF1->(FieldPos("F1_FIMP"))<>0 .And. Alltrim(aXML[nX][8])$"1,3,4,6"
...
Local nSoma       
Local nZ		    := 0 
Local nMaxCod	    := 15 //-- WHB ALTERADO DE 10 PARA 15
...
If !Empty("oDet[nX]:_IMPOSTO:_ICMS:_ICMSSN101:_VCREDICMSSN:TEXT") .And. lImpSimpN 
	...
	if SF3->(MsSeek(xFilial("SF3")+aNota[4]+aNota[5]))
		...
		aSimpNac[01] := Transform((nF3BaseIcm),"@ze 9,999,999,999,999.99")
		aSimpNac[02] := Transform((nF3ValIcm),"@ze 9,999,999,999,999.99")
...
For nZ := 1 To nLenDet
	...
	nMaxCod := Len(oDet[nX]:_Prod:_cProd:TEXT)        
...
For nY := 1 To nLenItens
	...
	Else
		...
		EndIf
		oDanfe:Say(nLinha, nAuxH2 + 2/* WHB INI */ +5 /* WHB FIM */, aAux[1][7][nY], oFont08:oFont) // QUANT
		nAuxH += aTamCol[7]
		If Empty(aAux[1][6][nY]) // UN
...
oDanfe:Say(859, 376, Alltrim(Transform(Val(oTotal:_ICMSTOT:_vNF:TEXT),"@ze 9,999,999,999,999.99")) ,oFont18N:oFont)
