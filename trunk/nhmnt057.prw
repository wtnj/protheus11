...
aAdd(aRotina,{ " Imprimir   ", "U_MNT57PRN()" , 0 , 6})
...
Private aFabr     := {"WHB I = Usinagem","WHB II = Fundição","WHB III = Usinagem","WHB IV = Forjaria","WHB V = Virabrequim"}
	...
	/* Inseridos os logins CLAYTONJS/FERNANDOMI a pedido do Fernando M Iida (05/09/2012) - GUILHERMEDC*/
		If !AllTrim(Upper(cUserName))$"MARCIOJF/GLEISONJP/SILMARAM/JOSEMF/CLAYTONJS/FERNANDOMI/GUILHERMEDC/ALEXANDREFL"
...
Static Function fRestricao()
	...
	If !AllTrim(Upper(cUserName))$"ADAIRJS/MARCIOJF/CLAYTONJS/SERGIOPS/MARCOSY/GLEISONJP/SILMARAM/TIAGOCO/JOAOFR/JOSEMF/FERNANDOMI/GUILHERMEDC/ALEXANDREFL/ADMIN/ADMINISTRADOR"
		Alert("Usuário sem permissão! Favor entrar em contato com o setor de Ferramentaria")
		Return.f.
	EndIF
...
//===============================================//
// Configuração do Ambiente para Impressão Cupom //
//===============================================//
User Function NHPRN057()

Local 	lFlag		:=	.T.
Private cBmpLogo	:= "\system\logo_whb.bmp"
Private cBmpLinha	:= "\system\LinhaVerde.bmp"

Private oFnt06		:= TFont():New("Courier New",,06,,.f.,,,,,.f.)
Private oFnt06n  	:= TFont():New("Courier New",,06,,.t.,,,,,.f.)
Private oFnt07		:= TFont():New("Courier New",,07,,.f.,,,,,.f.)
Private oFnt07n		:= TFont():New("Courier New",,07,,.t.,,,,,.f.)
Private oFnt08		:= TFont():New("Courier New",,08,,.f.,,,,,.f.)
Private oFnt08n		:= TFont():New("Courier New",,08,,.t.,,,,,.f.)
Private oFnt09		:= TFont():New("Courier New",,09,,.f.,,,,,.f.)
Private oFnt09n		:= TFont():New("Courier New",,09,,.t.,,,,,.f.)
Private oFnt10		:= TFont():New("Courier New",,10,,.f.,,,,,.f.)
Private oFnt10n		:= TFont():New("Courier New",,10,,.t.,,,,,.f.)
Private oFnt11		:= TFont():New("Courier New",,11,,.f.,,,,,.f.)
Private oFnt11n		:= TFont():New("Courier New",,11,,.t.,,,,,.f.)
Private oFnt12		:= TFont():New("Courier New",,12,,.f.,,,,,.f.)
Private oFnt12n		:= TFont():New("Courier New",,12,,.t.,,,,,.f.)
Private oFnt13		:= TFont():New("Courier New",,13,,.f.,,,,,.f.)
Private oFnt13n		:= TFont():New("Courier New",,13,,.t.,,,,,.f.)
Private oFnt14		:= TFont():New("Courier New",,14,,.f.,,,,,.f.)
Private oFnt14n		:= TFont():New("Courier New",,14,,.t.,,,,,.f.)
Private oFnt15		:= TFont():New("Courier New",,15,,.f.,,,,,.f.)
Private oFnt15n		:= TFont():New("Courier New",,15,,.t.,,,,,.f.)
Private oFnt16		:= TFont():New("Courier New",,16,,.f.,,,,,.f.)
Private oFnt16n		:= TFont():New("Courier New",,16,,.t.,,,,,.f.)
Private oFnt17		:= TFont():New("Courier New",,17,,.f.,,,,,.f.)
Private oFnt17n		:= TFont():New("Courier New",,17,,.t.,,,,,.f.)
Private oFnt18		:= TFont():New("Courier New",,18,,.f.,,,,,.f.)
Private oFnt18n		:= TFont():New("Courier New",,18,,.t.,,,,,.f.)
Private oFnt19		:= TFont():New("Courier New",,19,,.f.,,,,,.f.)
Private oFnt19n		:= TFont():New("Courier New",,19,,.t.,,,,,.f.)
Private oFnt20		:= TFont():New("Courier New",,20,,.f.,,,,,.f.)
Private oFnt20n		:= TFont():New("Courier New",,20,,.t.,,,,,.f.)
Private oFnt22		:= TFont():New("Courier New",,22,,.f.,,,,,.f.)
Private oFnt22n		:= TFont():New("Courier New",,22,,.t.,,,,,.f.)
Private nLinInc		:= 02*5
Private nColInc		:= 02*6
Private nRecNo		:= 0
Private nMaxItem	:= 40
Private nLin        := 03
Private nCol        := 03
...
oPrn := TMSPrinter():New("ORDEM DE SERVIÇO PARA FERRAMENTARIA")//Cria Objeto para impressao Grafica
...
//=================================//
// Impressão do Cabeçalho do Cupom //
//================================//
Static Function sfCabec()

oPrn:Box(nLinInc,nColInc,nLin*45,nCol*270) //Box do Cabeçalho
oPrn:SayBitmap(nLin*10,nColInc+nCol,cBmpLogo,nCol*18,nLin*7)//Logo Marga ->SayBitmap(Linha,Coluna,Figura,Coluna,Linha)

oPrn:Say (nLin*010,nCol*03, padc(" Empresa:" + Substr(Alltrim(SM0->M0_NOME),1,12) + "/Filial:" + SubStr(Alltrim(SM0->M0_FILIAL),1,12),40), oFnt09n, 100)
oPrn:Say (nLin*020,nCol*03, padc('O.S. PARA FERRAMENTARIA DA FORJARIA',40)                                                               , oFnt10n, 100)
oPrn:Say (nLin*030,nCol*05,' Hora:'+ Time()+'      Emissão:'+Dtoc(Date())                                                                , oFnt10n, 100)

oPrn:Say (nLin*050,nCol*05,'Número..........: ' + ZER->ZER_ORDEM                                                                         , oFnt08n, 100)
oPrn:Say (nLin*060,nCol*05,'Data Pedido.....: ' + Dtoc(ZER->ZER_DATINI)                                                                  , oFnt08n, 100)
oPrn:Say (nLin*070,nCol*05,'Prazo de Entrega: ' + Dtoc(ZER->ZER_DATPRV)                                                                  , oFnt08n, 100)

oPrn:Say (nLin*080,nCol*05,'Solicitante.....: ' + Posicione("QAA",1,xFilial("QAA")+ZER->ZER_SOLIC,"QAA_NOME")                            , oFnt08n, 100)
oPrn:Say (nLin*090,nCol*05,'Matricula.......: ' + ZER->ZER_SOLIC                                                                         , oFnt08n, 100)
oPrn:Say (nLin*100,nCol*05,'Centro de Custo.: ' + ZER->ZER_CC                                                                            , oFnt08n, 100)
oPrn:Say (nLin*110,nCol*05,'Setor...........: ' + Posicione("CTT",1,xFilial("CTT")+ZER->ZER_CC,"CTT_DESC01")                             , oFnt08n, 100)
oPrn:Say (nLin*120,nCol*05,'Quantidade......: ' + AllTrim(ZER->ZER_QUANT) + ' Máquina: '+ZER->ZER_CODBEM                                 , oFnt08n, 100)
oPrn:Say (nLin*130,nCol*05,'Situação........: ' + aSituac[Val(ZER->ZER_MAQSIT)]                                                          , oFnt08n, 100)
oPrn:Say (nLin*140,nCol*05,'Produto.........: ' + Alltrim(ZER->ZER_DISP)                                                                 , oFnt08n, 100)
oPrn:Say (nLin*150,nCol*05,'------------------ Descricao -------------------'                                                            , oFnt08n, 100)

nLin := nLin*160
For x:=1 To MlCount(ALLTRIM(ZER->ZER_DESC),100)
	
	oPrn:Say (nLin,nCol*5,MemoLine(ZER->ZER_DESC,100,x),oFnt08n)
	nLin += 25
...
If ZER->ZER_TERMIN=="S"
	...
	oPrn:Say (nLin,nCol*05,'------------------- Solucao --------------------'                                                            , oFnt08n, 100)

	nLin += 25	
	
	For x:=1 To MlCount(ALLTRIM(ZER->ZER_SOLUC),100)
		
		oPrn:Say (nLin,nCol*5,MemoLine(ZER->ZER_SOLUC,100,x),oFnt08n)
		nLin += 25
...
//================================================//
//Impressão da Lista de Itens da Ordem de Serviço //
//================================================//
Static Function sfDados()
Local nCont		:=	000
Local lFlag		:=	.T.

ZES->(DBSetOrder(1)) //Filal + Ordem
ZES->( DbGoTop() )
If ZES->(dbSeek(xFilial("ZES")+ZER->ZER_ORDEM))

	oPrn:Say (nLin,nCol*03,'------------------------------------------------' , oFnt10n, 100)
	nLin += 25
	oPrn:Say (nLin,nCol*03,'Insumos                                         ' , oFnt10n, 100)
	nLin += 25
	oPrn:Say (nLin,nCol*03,'Inicio          Fim              Duração        ' , oFnt10n, 100)
	nLin += 25
	oPrn:Say (nLin,nCol*03,'Observação                                      ' , oFnt10n, 100)
	nLin += 20
	oPrn:Say (nLin,nCol*03,'------------------------------------------------' , oFnt10n, 100)
	nLin += 20
	
	Do While ZES->(!EoF()) .And. ZES->ZES_ORDEM==ZER->ZER_ORDEM
		...
		If ZES->ZES_TIPO=="M"
			...
			If QAA->(dbSeek(xFilial("QAA")+ZES->ZES_CODIGO))
				...
				oPrn:Say (nLin,nCol*05,ZES->ZES_TIPO + ' ' + ZES->ZES_CODIGO + ' ' + SubStr(QAA->QAA_NOME,1,30) , oFnt08n, 100)
				nLin += 25
				
			EndIf
			
			oPrn:Say (nLin,nCol*05,Dtoc(ZES->ZES_DATINI) + ' ' + AllTrim(ZES->ZES_HORINI) + '  ' + Dtoc(ZES->ZES_DATFIM) + ' ';
			+ AllTrim(ZES->ZES_HORFIM) + '    '  + fDuracao(ZES->ZES_DATINI,ZES->ZES_DATFIM,ZES->ZES_HORINI,ZES->ZES_HORFIM), oFnt08n, 100)
		...
		ElseIf ZES->ZES_TIPO=="P"
			...
			If SB1->(dbSeek(xFilial("SB1")+ZES->ZES_CODIGO))
				...
				oPrn:Say (nLin,nCol*05,ZES->ZES_TIPO + ' ' + ZES->ZES_CODIGO + ' ' + SubStr(SB1->B1_DESC,1,30) + ' Quantidade: ' + ALLTRIM(STR(ZES->ZES_QUANT)), oFnt08n, 100)
		...
		For x:=1 To MlCount(ALLTRIM(ZES->ZES_DESC),35)
			...
			oPrn:Say (nLin,nCol*5,MemoLine(ZES->ZES_DESC,35,x),oFnt08n)
			nLin += 25
...
BrwLegenda(OemToAnsi("OrdemALLTRIM(STR(ZES->ZES_QUANT)) de Serviço de Ferramentaria"), "Legenda", aLegenda)
...
/*ALTERAÇÃO REALIZADA EM 05/09/2012 REFERENTE AO CHAMADO Nº: 037130 (FERNANDO M IIDA).
... acrescentar a opção "parada de máquina". Com a opção em questão selecionada, abrir um campo "parada de máquina"
com uma listagem de códigos, sendo permitida a alteração destas os usuários "CLAYTONJS", "ROBERTOU" e "FERNANDOMI".
(Posteriormente solicitado o login "MAICONMT" também.)

REALIZADA POR GUILHERME DEMÉTRIO CAMARGO.
*/	
	oSay16 := TSay():New(82,10,{||"Parada Maq."},oDlg,,,,,,.T.,,)
	oGet16 := tGet():New(80,50,{|u| if(Pcount() > 0, cCodPar := u,cCodPar)},oDlg,40,8,"@!",{||fValPar()},;
	,,,,,.T.,,,{||Substr(cAguard,1,1)=="5" .AND. (nPar==3 .Or. nPar==4)},,,,,,"ZF2","cCodPar")
	
	oGet17 := tGet():New(80,91,{|u| if(Pcount() > 0, cDesPar := u,cDesPar)},oDlg,132,8,"@!",{||.T.},;
	,,,,,.T.,,,{||.F.},,,,,,,"cDesPar")

	oSay10 := TSay():New(94,10,{||"Fornecedor"},oDlg,,,,,,.T.,,)
	oGet9  := tGet():New(92,50,{|u| if(Pcount() > 0, cFornec := u,cFornec)},oDlg,40,8,"@!",{||fIForn()},;
		,,,,,.T.,,,{||Substr(cAguard,1,1)=="2" .AND. (nPar==3 .Or. nPar==4)},,,,,,"SA2","cFornec")
	oGet10 := tGet():New(92,91,{|u| if(Pcount() > 0, cLoja := u,cLoja)},oDlg,10,8,"@!",{||fILoja()},;
		,,,,,.T.,,,{||Substr(cAguard,1,1)=="2" .AND. (nPar==3 .Or. nPar==4)},,,,,,,"cLoja")
	oGet11 := tGet():New(92,104,{|u| if(Pcount() > 0, cDesFor := u,cDesFor)},oDlg,120,8,"@!",{||.T.},;
		,,,,,.T.,,,{||.F.},,,,,,"","cDesFor")

	oSay11 := TSay():New(106,10,{||"Data Saida"},oDlg,,,,,,.T.,,)
	oGet12 := tGet():New(104,50,{|u| if(Pcount() > 0, dDatSai := u,dDatSai)},oDlg,50,8,"@!",{||.T.},;
		,,,,,.T.,,,{||Substr(cAguard,1,1)=="2" .AND. (nPar==3 .Or. nPar==4)},,,,,,,"dDatSai")

	oSay12 := TSay():New(106,120,{||"Prev. Retorno"},oDlg,,,,,,.T.,,)
	oGet13 := tGet():New(104,160,{|u| if(Pcount() > 0, dDatRet := u,dDatRet)},oDlg,50,8,"@!",{||.T.},;
		,,,,,.T.,,,{||Substr(cAguard,1,1)=="2" .AND. (nPar==3 .Or. nPar==4)},,,,,,,"dDatRet")

	oSay13 := TSay():New(118,10,{||"Desc. Aguarda"},oDlg,,,,,,.T.,,)
	oGet14 := tGet():New(116,50,{|u| if(Pcount() > 0, cAgDesc := u,cAgDesc)},oDlg,120,8,"@!",{||.T.},;
		,,,,,.T.,,,{||Substr(cAguard,1,1)=="4" .AND. (nPar==3 .Or. nPar==4)},,,,,,,"cAgDesc")

	oSay14  := TSay():New(130,10,{||"Descrição"},oDlg,,,,,,.T.,CLR_HBLUE,)
	oMemo1  := tMultiget():New(128,50,{|u|if(Pcount()>0,cDesc:=u,cDesc)},;
	oDlg,240,40,,,,,,.T.,,,{|| nPar==3 .Or. nPar==4 })
    
	oSay15 := TSay():New(174,10,{||"Máquina"},oDlg,,,,,,.T.,CLR_HBLUE,)
	oGet15 := tGet():New(172,50,{|u| if(Pcount() > 0, cMaq := u,cMaq)},oDlg,70,8,"@!",{||fValMaq()},;
		,,,,,.T.,,,{||.T.},,,,,,"ST9","cMaq")
		
	oGet18 := tGet():New(172,125,{|u| if(Pcount() > 0, cDesMaq := u,cDesMaq)},oDlg,140,8,"@!",{||.T.},;
		,,,,,.T.,,,{||.F.},,,,,,,"cDesMaq")

	oGroup := tGroup():New(194,05,196,295,,oDlg,,,.T.)

	oBtn1 := tButton():New(200,230,"Ok",oDlg,bOk,30,10,,,,.T.)
	oBtn2 := tButton():New(200,265,"Cancelar",oDlg,{||oDlg:End()},30,10,,,,.T.)
...
//³ VALIDA A PARADA DE MÁQUINA ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
Static Function fValPar()
	ZF2->(DbSetOrder(1))
	ZF2->(DbSeek(xFilial("ZF2")+cCodPar+ZF2_DESCRI))
	
	If ZF2->(!Found())
		Alert("Parada de Maquina não encontrada!")
		Return .F.
	Else
		cDesPar := ZF2->ZF2_DESCRI
		oGet17:Refresh()
	EndIf
...
Static Function fICarrega()
	...
	ZF2->(DbSetOrder(1))
	ZF2->(DbSeek(xFilial("ZF2")+cCodPar+ZF2->ZF2_DESCRI))
	cDesPar := ZF2->ZF2_DESCRI
...
*/
	...
	// Adicionado "Produto" em 11/01/2013
	@Prow()   ,060 psay "Produto:  "+Alltrim(ZER->ZER_DISP)
