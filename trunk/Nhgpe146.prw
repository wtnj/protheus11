...
±±ºPrograma  ³NHGPE146  ºAutor  ³Marcos R Roquitski  º Data ³  17/11/09   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Relatorio de Liquidos 13o. Salario - 2a. Parcela           .º±±
±±º          ³                                                            º±±
...
cDesc2    := OemToAnsi("liquidos dos pagamentos de terceiros 13o. Salario - 2a. Parcela")
cDesc3    := OemToAnsi("")
...
Titulo  := "Liquido terceiro: 13o. Salario - 2a. Parcela "+MesExtenso(Month(dDataBase))+"/"+Alltrim(Str(Year(dDataBase)))
...
If TMP->(Recno() > 0)
	...
	While TMP->(!eof())
		...
		@ Prow()   , 100 PSAY TMP->ZRF_VALOR  Picture "@E 99,999,999.99"
		_nTotLiq += TMP->ZRF_VALOR
		_nTotGer += TMP->ZRF_VALOR
		TMP->(DbSkip())
...
If TMT->(Recno()) > 0
	...
	While TMT->(!eof())
		...
		@ Prow()   , 100 PSAY TMT->ZRF_VALOR  Picture "@E 99,999,999.99"
		_nTotLiq += TMT->ZRF_VALOR
		_nTotGer += TMT->ZRF_VALOR
		TMT->(DbSkip())
...
If TMD->(Recno()) > 0
	...
	While TMD->(!eof())
		...
		@ Prow()   , 100 PSAY TMD->ZRF_VALOR  Picture "@E 99,999,999.99"
		_nTotLiq += TMD->ZRF_VALOR
		_nTotGer += TMD->ZRF_VALOR
		TMD->(DbSkip())
...
cQuery := "SELECT RA.ZRA_MAT,RA.ZRA_CPFCGC,RA.ZRA_BANCO,RA.ZRA_AGENCI,RA.ZRA_CONTA,RA.ZRA_FAVORE,RF.ZRF_VALOR FROM " + RetSqlName('ZRA') + " RA, " +  RetSqlName('ZRF') + " RF "
cQuery += "WHERE RF.D_E_L_E_T_ = ' ' " 
cQuery += "AND RA.D_E_L_E_T_ = ' ' " 
cQuery += "AND RF.ZRF_PD = '799' "
cQuery += "AND RA.ZRA_FIM = ' ' " 
cQuery += "AND RA.ZRA_BANCO = '001' " 
cQuery += "AND RF.ZRF_MAT = RA.ZRA_MAT " 
