...
±±ºPrograma  ³NHGPE216  ºAutor  ³Marcos R Roquitski  º Data ³  26/10/10   º±±
...
If !Empty(TMP0->RA_MAT) 
	If SM0->M0_CODIGO == "FN"	// Fundicao
		cArqDep := "C:\RELATO\FNHO" + Substr(Dtos(dDataBase),5,4) + ".REM" // Deposito
	Else
		cArqDep := "C:\RELATO\NHHO" + Substr(Dtos(dDataBase),5,4) + ".REM" // Deposito
	Endif	
	cFnl    := CHR(13)+CHR(10)
		...
		** SEGMENTO E - processa Desconto/Debito
		...
		If _n723Ir > 0
			...
			** SEGMENTO F - processa Mensagem
			...
			cLin := cLin + Space(03)
			cLin := cLin + '2'				
			cLin := cLin + '723 I.R. ADIANTAMENTO BS R$ ' + TRANSFORM(_n723Ir,'@E 999,999.99') + Space(184)
