...
ฑฑบPrograma  ณ NHEST215 บAutor ณDouglas de S. Douradoบ Data ณ 06/11/2012บ  ฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ                            
ฑฑบDesc.     ณ Envia E-Mails p/ grupos (Schedule)                         บฑฑ                                     
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบUso       ณ AP11 - ESTOQUE E CUSTOS                                    บฑฑ
ฑฑบ	         ณ TABELAS: ZG1(CAB), ZG2(EMAILS) E ZG3(RESP.)   			  บฑฑ
...
Private oMail , _cCusto, nDiasAb 
	...
	// Pega todas as rnc's em aberto e com no mํnimo 3 dias jแ em aberto
	cQuery := " SELECT * " 
	...
	ZG1->(DBSETORDER(3)) 
		...
		While !TEMP->(Eof()) 
			  ...
			  // Busca o CC da pe็a da RNC que estแ selecionada	
		      _cCusto := Posicione("SB1",1,xFilial("SB1") + TEMP->ZF4_PROD,"B1_CC")
	          
			  // Verifica hแ quantos dias a rnc estแ em aberto
    ...
    IF ZG1->(DbSeek(xFilial("ZG1") + __cCusto )) 
			...
			While !ZG1->(Eof())
		   	    ...
		   	    If AllTrim(ZG1->ZG1_CC) == AllTrim(__cCusto) .AND. AllTrim(ZG1->ZG1_STAT) == 'A'  // Verifica o Centro de Custo
				      ...
				      // Verifica se o grupo encontrado se encaixa no perํodo de dias em aberto da RNC
		    ...
		    // Caso nใo ache nenhum grupo que se encaixe nesse periodo com este CC, nao manda email
		    If len(_aGrupos) == 0
		       Return	
		    EndIf
		    
		    // Com base no array de grupos (_aGrupos) pega os emails e verificada, para nใo ocorrer duplica็ใo.
			...
			oMail:cMsg += 'A RNC '+_cRNC+' estแ em aberto hแ '+STR(_Dias)+' dias, favor verificar. <br/>'
			oMail:cMsg += '<br/>'
			oMail:cMsg += '<span style="text-decoration:underline">INFORMAวีES:<br/></span>'
			oMail:cMsg += '<br/>'
			oMail:cMsg += 'RNC aberta em '+DtoC(_dData)+' por '+_cLogin+' referente เ<br/>'
			oMail:cMsg += ''+STR(_cSaldo)+' pe็as suspeitas pendentes para destina็ใo do produto '+_cProd+' que foram do armazem '+_cAlmOrg+' para o armazem '+_cAlmDes+'.<br/>'
			oMail:cMsg += '</td>'
			oMail:cMsg += '</tr>'
			oMail:cMsg += '<tr>'			
			oMail:cMsg += '<td align="center" style="color:#FFFFFF;font-size:20px; font-family:arial">'
