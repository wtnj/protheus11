...
±±³Programa  ³ NHEST061        ³ Marcos R. Roquitski   ³ Data ³ 26/02/03 ³±±
...
ALTERAÇÃO NA POSIÇÃO DAS INFORMAÇÕES NA ETIQUETA.

REALIZADO POR FELIPE CICONINI.

*/
#include "rwmake.ch"      
#INCLUDE "TOPCONN.CH"

User Function NhEst061()  
...
//-----------------------------------------------------------------\\            
	    ...
	    oPr:Say(nLin+070,060,"WHB",oFonteN8)
	    oPr:Say(nLin+085,610,"CONTROLE",oFonteN2)

	    oPr:Say(nLin+070,845,"WHB",oFonteN8)
	    oPr:Say(nLin+085,1395,"CONTROLE",oFonteN2)
		
		oPr:Say(nLin+075,1640,"WHB",oFonteN8)
		...
		oPr:Say(nLin+145,060,"Fornecedor:",oFonteN1)
	    oPr:Say(nLin+145,210,cC19+" "+Upper(cC12), oFonteN1)
//	    oPr:Say(nLin+155,330,Upper(cC12), oFonteN1) //NOME CLIENTE

	    
   	    oPr:Say(nLin+145,0845,"Fornecedor:",oFonteN1)
	    oPr:Say(nLin+145,0995,cC29+" "+Upper(cC22),oFonteN1)        //COD CLIENTE
//	    oPr:Say(nLin+155,1115,Upper(cC22),oFonteN1) //NOME CLIENTE
	    
	    oPr:Say(nLin+145,1630,"Fornecedor:",oFonteN1)
	    oPr:Say(nLin+145,1800,cC39+" "+Upper(cC32),oFonteN1)         //COD CLIENTE
//	    oPr:Say(nLin+155,1920,Upper(cC32),oFonteN1)  //NOME CLIENTE


	    oPr:Say(nLin+130,510,DtoC(StoD(cC13)),oFonteN5)
	    oPr:Say(nLin+130,1295,DtoC(StoD(cC23)),oFonteN5)
	    oPr:Say(nLin+130,2100,DtoC(StoD(cC33)),oFonteN5)


	    oPr:Say(nLin+175,0060,cC15,oFonteN6)   	//codigo produto
	    oPr:Say(nLin+175,0845,cC25,oFonteN6)	//codigo produto
	    oPr:Say(nLin+175,1630,cC35,oFonteN6)	//codigo produto
                            

	    oPr:Say(nLin+190,610,"NF:",oFonteN1)
	    oPr:Say(nLin+190,650,cC18,oFonteN1)
                        

	    oPr:Say(nLin+190,1395,"NF:",oFonteN1)
	    oPr:Say(nLin+190,1435,cC28,oFonteN1)
	                 
	    
	    oPr:Say(nLin+190,2200,"NF:",oFonteN1)
	    oPr:Say(nLin+190,2240,cC38,oFonteN1)
	    
	    
	    oPr:Say(nLin+235,0260,cC14,oFonteN3)	//denominacao da peca
	    oPr:Say(nLin+235,1045,cC24,oFonteN3)	//denominacao da peca 
	    oPr:Say(nLin+235,1850,cC34,oFonteN3)	//denominacao da peca
	     

	    oPr:Say(nLin+255,060,"Quantidade:",oFonteN1)
	    oPr:Say(nLin+270,060,Transform(cC17,"999999"),oFonteN7)
	   
		oPr:Say(nLin+255,845,"Quantidade:",oFonteN1)
	    oPr:Say(nLin+270,845,Transform(cC27,"999999"),oFonteN7)
	    
	    oPr:Say(nLin+255,1630,"Quantidade:",oFonteN1)
	    oPr:Say(nLin+270,1630,Transform(cC37,"999999"),oFonteN7)
...
Static Function Gerando()
   ...
   cQuery += "         AND A2.A2_LOJA = D1.D1_LOJA AND A2.A2_FILIAL = '"+XFILIAL("SA2")+"' ) "
   cQuery += "   ELSE "
   cQuery += "        (SELECT A1.A1_NREDUZ "
   cQuery += "         FROM " +RetSqlName('SA1') +" A1 (NOLOCK)" 
   cQuery += "         WHERE A1.D_E_L_E_T_ = ' ' AND A1.A1_COD = D1.D1_FORNECE "
   cQuery += "         AND A1.A1_LOJA = D1.D1_LOJA AND A1.A1_FILIAL = '"+XFILIAL("SA1")+"') "
   cQuery += "END "   
   ...
   cQuery += " LEFT JOIN "+ RetSqlName( 'SB5' ) +" QEK (NOLOCK) ON "
