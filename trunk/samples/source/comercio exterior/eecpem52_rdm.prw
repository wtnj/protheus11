...
Static aMESES := {"ENERO","FEBRERO","MARZO","ABRIL","MAYO","JUNIO","JULIO","A1GOSTO","SEPTIEMBRE","OCTUBRE","NOVIEMBRE","DICIEMBRE"}
...
Begin Sequence   
   ...
   HEADER_P->AVG_C02_20:= AllTrim(EEC->EEC_PEDREF)
   ...
   Do While ! EE9->(EOF()) .AND.;
      ...
      IF nITENSEE9 > 1
         APPENDDET()
      ENDIF
      cMemo := AllTrim(MSMM(EE9->EE9_DESC,AVSX3("EE9_VM_DES",AV_TAMANHO)))
...
Begin Sequence
   ...
   Define MsDialog oDlg Title WorkId->EEA_TITULO From 9,0 To 38,80 Of oMainWnd
     ...
     oMark3 := EECMensagem(EEC->EEC_IDIOMA,"#",{18,3,175,312},,,,oDlg)
...
Begin Sequence
   ...
   DEFINE MSDIALOG oDlg TITLE ALLTRIM(WORKID->EEA_TITULO) FROM 0,0 TO 240,357 OF oMainWnd PIXEL
      ...
      oFLD := TFolder():New(13,0,{STR0041,STR0042},{"IMP","EXP"},oDlg,,,,.T.,.F.,180,108) //"&Importador"###"&Exportador"
      IF nP_LINHA > 0
