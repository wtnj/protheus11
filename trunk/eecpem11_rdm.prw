...
Obs.            :
***********************************************************************
Alteracao       : Alessandro J. Porta (AJP)
Data/Hora       : 18/10/06 - 11H00
Objetivo        : Demonstrar o valor FOB unitario da mercadoria
Demonstrar o valor da Comissão do Agente
Puxar o cliente como consignatario quando consignatario em branco
Puxar o codigo do produto do campo Part-number
*/
#include "EEC.CH"
#define NUMLINPAG 23
...
EEB->(dbSetOrder(1)) //EEB_FILIAL+EEB_PEDIDO+EEB_OCORRE+EEB_CODAGE+EEB_TIPOAG   AJP 18/10/06
...
IF !EMPTY(EEC->EEC_EXPORT) .AND. ;
	...
	If !Alltrim(Upper(cusername))$'EDUARDOM'
	...
	ElseIf Alltrim(Upper(cusername))$'EDUARDOM'
		cEXP_CONTATO := EECCONTATO(CD_SA2,EEC->EEC_EXPORT,EEC->EEC_EXLOJA,"2",1)  //nome do contato seq 1
		cEXP_FONE    := EECCONTATO(CD_SA2,EEC->EEC_EXPORT,EEC->EEC_EXLOJA,"2",4)  //fone do contato seq 1
		cEXP_FAX     := EECCONTATO(CD_SA2,EEC->EEC_EXPORT,EEC->EEC_EXLOJA,"2",7)  //fax do contato seq 1
		cEXP_CARGO   := EECCONTATO(CD_SA2,EEC->EEC_EXPORT,EEC->EEC_EXLOJA,"2",2)  //CARGO
	EndIf
...
ELSE
	...
	ElseIf Alltrim(Upper(cusername))$'EDUARDOM'
		cEXP_CONTATO := EECCONTATO(CD_SA2,EEC->EEC_FORN,EEC->EEC_FOLOJA,"2",1,EEC->EEC_RESPON)  //nome do contato seq 1
		cEXP_FONE    := EECCONTATO(CD_SA2,EEC->EEC_FORN,EEC->EEC_FOLOJA,"2",4,EEC->EEC_RESPON)  //fone do contato seq 1
		cEXP_FAX     := EECCONTATO(CD_SA2,EEC->EEC_FORN,EEC->EEC_FOLOJA,"2",7,EEC->EEC_RESPON)  //fax do contato seq 1
		cEXP_CARGO   := EECCONTATO(CD_SA2,EEC->EEC_FORN,EEC->EEC_FOLOJA,"2",2,EEC->EEC_RESPON)  //CARGO
	EndIf
...
HEADER_P->AVG_C04_60:=ALLTRIM("TEL:"+AllTrim(cEXP_FONE)+" FAX:"+AllTrim(cEXP_FAX)) //"TEL.: "###" FAX: "

HEADER_P->AVG_C21_20:=TRANSFORM(Posicione("SA2",1,xFILIAL("SA2")+EEC->EEC_FORN+EEC->EEC_FOLOJA,"A2_CGC"),AVSX3("A2_CGC",AV_PICTURE))
...
If lUnidade
	...
	IF ! EE2->(Dbseek(xFilial("EE2")+"8"+"*"+EEC->EEC_IDIOMA+cUnPeso))
		MsgStop("Unidade de Medida "+cUnPeso+" não cadastrada no idioma"+EEC->EEC_IDIOMA,"Aviso") //"Unidade de medida "###" nào cadastrada em "###"Aviso"
	EndIf
...
nValCom := 0
//AJP 18/10/06 - Busca comissão
EEB->(dbSeek(xFilial("EEB")+EEC->EEC_PREEMB+AvKey("Q","EEB_OCORRE")))
While !EEB->(EOF()) .And. EEB->EEB_FILIAL == xFilial("EEB") .And. EEB->EEB_PEDIDO == EEC->EEC_PREEMB .AND. ;
	EEB->EEB_OCORRE == Avkey("Q","EEB_OCORRE")
	If Left(EEB->EEB_TIPOAG,1)=="3"
		If nValCom = 0
			cPerCom := If(EEB->EEB_TIPCVL == "1",Transform(EEB->EEB_VALCOM,"@E 99.99")+"%","")
		Else
			cPerCom := ""
		End If
		nValCom += EEB->EEB_TOTCOM
	EndIf
	EEB->(dbSkip())
End Do
HEADER_P->AVG_C19_20 := cPerCom
HEADER_P->AVG_C20_20 := AllTrim(Transf(nValCom,cPict))
...
IF Posicione("SYJ",1,xFilial("SYJ")+EEC->EEC_INCOTE,"YJ_CLFRETE") $ "1YS"
	...
	IF lNcm
		...
		If lUnidade .And. !lDescUnid
			...
			IF ! EE2->(Dbseek(xFilial("EE2")+"8"+"*"+EEC->EEC_IDIOMA+cUnQtde))
				MsgStop("Unidade de Medida "+cUnQtde+" não cadastrada em "+EEC->EEC_IDIOMA,"Aviso") //"Unidade de medida "###" nào cadastrada em "###"Aviso"
			EndIf
			...
			IF ! EE2->(Dbseek(xFilial("EE2")+"8"+"*"+EEC->EEC_IDIOMA+cUnPreco))
				MsgStop("Unidade de Medida "+cUnPreco+" não cadastrada em "+EEC->EEC_IDIOMA,"Aviso") //"Unidade de medida "###" nào cadastrada em "###"Aviso"
			EndIf
		...
		ElseIf !lUnidade
			IF cUnidade <> EE9->EE9_UNIDAD
				...
				IF ! EE2->(Dbseek(xFilial("EE2")+"8"+"*"+EEC->EEC_IDIOMA+EE9->EE9_UNIDAD))
					MsgStop("Unidade de Medida "+EE9->EE9_UNIDAD+" não cadastrada em "+EEC->EEC_IDIOMA,"Aviso") //"Unidade de medida "###" nào cadastrada em "###"Aviso"
				Endif
				DETAIL_P->AVG_C06_20 := AllTrim(EEC->EEC_MOEDA)+"/"+EE2->EE2_DESCMA
	...
	While EE9->(!Eof() .And. EE9_FILIAL == xFilial("EE9")) .And.;
		...
		ElseIf !lUnidade
			...
			IF cUnidade <> EE9->EE9_UNIDAD
				...
				IF ! EE2->(Dbseek(xFilial("EE2")+"8"+"*"+EEC->EEC_IDIOMA+EE9->EE9_UNIDAD))
					MsgStop("Unidade de Medida "+EE9->EE9_QUANTID+" não cadastrada em "+EEC->EEC_IDIOMA,"Aviso") //"Unidade de medida "###" nào cadastrada em "###"Aviso"
				Endif
		...
		// DETAIL_P->AVG_C02_20 := Transf(EE9->EE9_COD_I,AVSX3("EE9_COD_I",6)) AJP 18/10/06
		DETAIL_P->AVG_C02_20 := EE9->EE9_PART_N
		...
		If lUnidade
			DETAIL_P->AVG_C06_20 := AllTrim(Transf(AVTransUnid(EE9->EE9_UNIDAD,cUnPreco,EE9->EE9_COD_I,EE9->EE9_PRECOI,.f.),cPictPreco))
		Else
			//DETAIL_P->AVG_C06_20 := AllTrim(Transf(EE9->EE9_PRECO,cPictPreco)) AJP 18/10/06
			DETAIL_P->AVG_C06_20 := AllTrim(Transf(EE9->EE9_PRECOI,cPictPreco))
		Endif
		// FJH
		...
		Else
			//DETAIL_P->AVG_C07_20 := AllTrim(Transf(Round(EE9->EE9_PRECO*EE9->EE9_SLDINI,2),cPict)) AJP 18/10/06
			DETAIL_P->AVG_C07_20 := AllTrim(Transf(Round(EE9->EE9_PRECOI*EE9->EE9_SLDINI,2),cPict))
		EndIf
		...
		Else
			//nTotal   := nTotal  + Round(EE9->EE9_PRECO*EE9->EE9_SLDINI,2) AJP 18/10/06
			nTotal   := nTotal  + Round(EE9->EE9_PRECOI*EE9->EE9_SLDINI,2)
		EndIf
...
While EEM->(!Eof() .And. EEM_FILIAL == xFilial()) .And.;
	...
	SysRefresh()
	IF Empty(cNotas)
		cNotas := cNotas+" Notas Fiscais: " //"Notas Fiscais:"
	Endif
...
{"WKCODIGO",,"Código"},; //"Código"
{"WKDESCR",,"Descrição"}} //"Descrição"
			...
			if(nTela==4,oMark3:oBrowse:Hide(),;
				...
				Else
					...
					if(nTela==3,oMark2:oBrowse:Hide(),;
						...
						oFld := TFolder():New(1,1,If(lUnidade,{"Documentos Para","Notify´s","Observação","Mensagens","Configurações"},{"Documentos Para","Notify´s","Observação","Mensagens"}),; //"Configurações"
						If(lUnidade,{"IPC","IBC","OBS","MEN","CFG"},{"IPC","IBC","OBS","MEN"}),oDlg,,,,.T.,.F.,315,127) //"Documentos Para"###"Notify's"###"Mensagens"###"Observações"
							...
							If !lUnidade
								@ 10,001 SAY "Imprime N.C.M" SIZE 232,10 PIXEL OF oFldDoc //"Imprime N.C.M."
								
								oYes := TCheckBox():New(10,42,"Sim",{|x| If(PCount()==0, lNcm,Eval(bSet, x,oNo ))},oFldDoc,21,10,,,,,,,,.T.) //"Sim"
								oNo  := TCheckBox():New(10,65,"Não",{|x| If(PCount()==0,!lNcm,Eval(bSet,!x,oYes))},oFldDoc,21,10,,,,,,,,.T.) //"Não"
								
								@ 10,100 SAY "Imprime Peso Bruto"  SIZE 232,10 PIXEL OF oFldDoc //"Imprime Peso Bruto"
								
								oYesP := TCheckBox():New(10,157,"Sim",{|x| If(PCount()==0, lPesoBru,Eval(bSetP, x,oNoP ))},oFldDoc,21,10,,,,,,,,.T.) //"Sim"
								oNoP  := TCheckBox():New(10,180,"Não",{|x| If(PCount()==0,!lPesoBru,Eval(bSetP,!x,oYesP))},oFldDoc,21,10,,,,,,,,.T.) //"Não"
							Else
								nPosLin := 10
							EndIf
							
							//     M->cCONTATO   := EEC->EEC_RESPON  //cEXP_CONTATO
							//     M->cEXP_CARGO := "EXPORT MANAGER"
							M->cCONTATO   := cEXP_CONTATO
							M->cEXP_CARGO := cEXP_CARGO
							
							@ nPosLin,001 SAY "Assinante" SIZE 232,10 PIXEL OF oFldDoc //"Assinante"
							@ nPosLin,043 GET M->cCONTATO SIZE 120,08 PIXEL OF oFldDoc
							
							@ nPosLin+10,001 SAY "Cargo" SIZE 232,10 PIXEL OF oFldDoc //"Cargo"
							@ nPosLin+10,043 GET M->cEXP_CARGO SIZE 120,08 PIXEL OF oFldDoc
							
							@ nPosLin+24,001 SAY "Doct. Para" SIZE 232,10 PIXEL OF oFldDoc //"Doct.Para"
							...
							If lUnidade
								...
								@ 05,03 To 60,310 LABEL "Unidades de Medida" OF oFldCFG PIXEL //"Unidades de Medida"
								
								@ 15,08 SAY "U.M Qtde.:" SIZE 50,07 OF oFldCFG PIXEL //"U.M. Qtde.:"
								@ 15,55 MSGET cUnQtde SIZE 20,07 F3 "SAH" OF oFldCFG PIXEL
								
								@ 27,08 SAY "U.M Preço.:" SIZE 50,07 OF oFldCFG PIXEL //"U.M. Preço.:"
								@ 27,55 MSGET cUnPreco SIZE 20,07 F3 "SAH" OF oFldCFG PIXEL
								
								@ 39,08 SAY "U.M Peso.:" SIZE 50,07 OF oFldCFG PIXEL //"U.M. Peso.:"
								@ 39,55 MSGET cUnPeso SIZE 20,07 F3 "SAH" OF oFldCFG PIXEL
								
								@ 65,03 To 100,310 LABEL "Impressão" OF oFldCFG PIXEL     //"Impressão"
								
								@ 75,08 SAY "Imprime N.C.M." SIZE 232,10 PIXEL OF oFldCFG //"Imprime N.C.M."
								
								oYes := TCheckBox():New(75,62,"Sim",{|x| If(PCount()==0, lNcm,Eval(bSet, x,oNo ))},oFldCFG,21,10,,,,,,,,.T.) //"Sim"
								oNo  := TCheckBox():New(75,85,"Não",{|x| If(PCount()==0,!lNcm,Eval(bSet,!x,oYes))},oFldCFG,21,10,,,,,,,,.T.) //"Não"
								
								@ 87,08 SAY "Imprime Peso Bruto" SIZE 232,10 PIXEL OF oFldCFG //"Imprime Peso Bruto"
								
								oYesP := TCheckBox():New(87,62,"Sim",{|x| If(PCount()==0, lPesoBru,Eval(bSetP, x,oNoP ))},oFldCFG,21,10,,,,,,,,.T.) //"Sim"
								oNoP  := TCheckBox():New(87,85,"Não",{|x| If(PCount()==0,!lPesoBru,Eval(bSetP,!x,oYesP))},oFldCFG,21,10,,,,,,,,.T.) //"Não"
									...
									*/
									...
									IF WkMsg->(!Eof())
										IF Empty(WkMsg->WKMARCA)
											...
											@ 05,05 SAY "Tipo Mensagem" PIXEL //"Tipo Mensagem"
