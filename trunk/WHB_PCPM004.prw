...
!Nome              ! WHB_PCPM004                                             !
+------------------+---------------------------------------------------------+
!Descrição         ! Firmar ordem de producao                                !
+------------------+---------------------------------------------------------+
!Autor             ! Rafael Ricardo Vieceli                                  !
+------------------+---------------------------------------------------------+
!Data de Criação   ! 23/08/2010                                              !
+------------------+---------------------------------------------------------+
!   ATUALIZACÕES                                                             !
+-------------------------------------------+-----------+-----------+--------+
...
!Nome              ! montaTela                                               !
+------------------+---------------------------------------------------------+
!Descrição         ! Funcao para montagem da tela de interface com usuario   !
+------------------+---------------------------------------------------------+
!Autor             ! Rafael Ricardo Vieceli                                  !
+------------------+--------------------------------------------------------*/
...
Local aProva := { "N=Não" , "S=Sim"}
Define MsDialog oDlg Title "Detalhes da Ordem de Produção" From 0,0 To 347,395 Pixel Style DS_MODALFRAME

	//Botoes
   	bEnchoice := EnchoiceBar(oDlg,{||lRet := .t.,oDlg:End()},{||Alert('Favor digitar os dados e confirmar!')})

	@ 015,005 To 65,195 Label " Detalhes da Ordem de Produção " Of oDlg Pixel
	@ 025,009 Say "Número OP:"    Size 035,010 Of oDlg Pixel
	@ 035,009 Say "Produto:"      Size 035,010 Of oDlg Pixel
	@ 045,009 Say "Descrição"     Size 035,010 Of oDlg Pixel
	@ 055,009 Say "Data Emissão:" Size 035,010 Of oDlg Pixel
	...
	@ 084,009 Say "Data Início:"  Size 035,010 Of oDlg Pixel
	...
	@ 108,009 Say "Hora Início:"  Size 035,010 Of oDlg Pixel
...
!Nome              ! fPeriodo                                                     !
+------------------+--------------------------------------------------------------+
!Descrição         ! Verifica se a o periodo informado não sobrepõe o período de  !
!                  ! outra OP.                                                    !
+------------------+--------------------------------------------------------------+
...
Local _lRet := .T. 
	...
	If !_lRet
    	Alert("O período informado já está sendo utilizado por outra OP! " + CHR(10)+CHR(13) + ;
    	      "Favor informar outro período!" + CHR(10)+CHR(13)  + CHR(10)+CHR(13) + ;
    	      "OP: "          +(cAl)->num       + CHR(10)+CHR(13) + ;
    	      "Data Início: " + DtoC((cAl)->di) + CHR(10)+CHR(13) + ;
    	      "Data Fim: "    + DtoC((cAl)->df) + CHR(10)+CHR(13) + ;
    	      "Hora Início: " + (cAl)->hi + CHR(10)+CHR(13) + ;
...
!Nome              ! fProxHora                                               !
+------------------+---------------------------------------------------------+
!Descrição         ! Funcao para trazer hora inicial da próxima op           !
+------------------+---------------------------------------------------------+
!Autor             ! João Felipe da Rosa / Mario C. Pietrzak                 !
+------------------+--------------------------------------------------------*/
/*
