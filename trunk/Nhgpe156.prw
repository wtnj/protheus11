...
±±³ Leandro Dr.³05/03/2009³005346³Compatibilizacao com o fonte GPER030.PRX  ³±±
±±³MauricioMR  ³12/03/2009³005391³ Somente demonstra o recibo TCF para meses³±±
±±³            ³          ³      ³ anteriores ou igual ao mes corrente      ³±±
±±³Tatiane V.M.³25/08/2009³020388³Compatibilizacao dos fontes  para aumento ³±±
±±³            ³          ³  2009³ do campo filial e gestão corporativa.    ³±±
±±ÀÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß*/
User Function Nhgpe156(lTerminal,cFilTerminal,cMatTerminal,cMesAnoRef,nRecTipo,cSemanaTerminal)
...
If lTerminal
	...
	If	cMesCorrente == cMesArqRef  .or. right(cMesCorrente,4)+left(cMesCorrente,2) == mesano(ddataref) .Or. ;
		...
		ElseIf Esc == 2 .and. !lNaoChkDFol
			...
			//-- Somente demonstra o Recibo no Terminal para Recibo do mes corrente ou dos meses anteriores
			If (lTerminal <> NIL .And. lTerminal) .and. ;
			   ( ( Right(cMesCorrente,4)+Left(cMesCorrente,2) ) < ( Right(cMesAnoRef,4)+Left(cMesAnoRef,2)  ) )
				Return( cHtml ) 
			Endif
		ElseIf Esc == 3
			If ( Right(cMesAnoRef,4)+Left(cMesAnoRef,2) > Right(cMesCorrente,4)+Left(cMesCorrente,2) ) .Or.;
...
@ LI,47 PSAY PADC(ALLTRIM(_ccSrc),20)
...
cDet += DescCc(_ccSrc,SRA->RA_FILIAL) + ' '
cDet += STR0016 + SRA->RA_CHAPA					//'CHAPA: '
@ Li,01 pSay cDet                     

LI +=2
@ Li,01 pSay 'Cod.    Descricao              Referencia   Vencimentos     Descontos'
Li += 1 //2
Return Nil
...
@ LI,35 PSAY STR0072
@ LI,70 PSAY STR0073 + Alltrim(SRA->RA_BCDEPSAL) + "-" + Alltrim(DescBco(SRA->RA_BCDEPSAL,SRA->RA_FILIAL))
@ LI,131 PSAY "|"
@ ++LI,00 PSAY "|"+REPLICATE("-",130)+"|"
...
@ LI,44 PSAY 'T. de Vencto'
@ LI,58 PSAY 'T. Descontos'
LI ++
...
Else
	nValSal := SRA->RA_SALARIO
EndIf
@ LI,01 PSAY nValSal PICTURE "@E 999,999.99"

If Esc = 1  // Bases de Adiantamento
    If cBaseAux = "S" .And. nBaseIr # 0
        @ LI,89 PSAY nBaseIr PICTURE "@E 999,999.99"
    Endif

ElseIf Esc = 2 .Or. Esc = 4  // Bases de Folha e 13o. 2o.Parc.
	If cBaseAux = "S"
		@ LI,12 PSAY nAteLim PICTURE "@E 999,999.99"

		If nBaseFgts # 0
			@ LI,24 PSAY nBaseFgts PICTURE "@E 999,999.99"
		Endif

		If nFgts # 0
			@ LI,36 PSAY nFgts PICTURE "@E 999,999.99"
		Endif

		If nBaseIr # 0
			@ LI,48 PSAY nBaseIr PICTURE "@E 999,999.99"
		Endif
		@ LI,60 PSAY nBaseIrfE PICTURE "@E 999,999.99"
	Endif
...
Li ++
@ LI,02 pSay 'Sal.Base    Sal.INSS B.Calc.Fgts    Fgts/Mes B.Calc.IRPF  Faixa IRRF'
Li ++
IF SRA->RA_BCDEPSAL # SPACE(8)
    ...
    @ LI,14 PSAY "-"
    @ LI,15 PSAY Alltrim(DESC_BCO)
	@ LI,40 PSAY "CONTA:" + Alltrim(SRA->RA_CTDEPSAL)
ENDIF
LI += 1
@ LI,05 PSAY " "
Return Nil
...
IF !( lTerminal )
	...
	cHtml += 	'&nbsp;&nbsp;&nbsp;&nbsp;'+ STR0098  + Desc_End	+'<br>'		//Endereço
