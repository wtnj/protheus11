...
-ALTERAÇÃO DA DISPOSIÇÃO NOS DADOS DA ETIQUETA

--28/03/2011
-FELIPE CICONINI VALCANAIA
-MUDANÇA DE IMPRESSÃO DE ETIQUETAS PARA PRODUTOS VW CONTROLADOS POR LOTE
*/

#include "rwmake.ch"      
#INCLUDE "TOPCONN.CH"

User Function NhEst004()  
...
Local nPagina := 1,;
	...
	oFonteG1 := TFont():New("Arial"		,,15,,.T.,,,,,.F.)
	   	...
	   	For i := 1 To nQtEtq
			...
			For j := 1 To nQtSal
				...
				Endif
	         	...
	         	aAdd(aEtiq,{cInsp,Substr(TMP->CLIENTE,1,18),TMP->D1_DTDIGIT,TMP->D1_DESCRI,TMP->D1_COD,TMP->LOTECTL,nQtImp,TMP->D1_DOC,TMP->D1_SERIE,TMP->D1_FORNECE+"/"+TMP->D1_LOJA})
	         	nQtEti--
	         	nEtiqImp++
   	...
   	For m := 1 To Len(aEtiq)                                                  
        ...
        cC15 := Space(10)   //D1 COD
        cC16 := Space(10)	//LOTE
        cC17 := 0			//QTD
        cC18 := Space(12)	//DOC/SERIE
        cC19 := Space(09)	//FORN/LOJA
...
//----------------------FORNEC/LOJA--------------------------------\\
		...
		oPr:Say(nLin+250,0100,"Fornecedor:",oFonteN1)
	    oPr:Say(nLin+290,0100,cC19, oFonteN1)
	    oPr:Say(nLin+290,0325,Upper(cC12), oFonteN1) //NOME CLIENTE
...
//-----------------------LOTE--------------------------------------\\
		...
		oPr:Say(nLin+335,0100,"LOTE: "+cC16,oFonteN1)

		oPr:Say(nLin+335,1300,"LOTE: "+cC26,oFonteN1)
//-----------------------DATA--------------------------------------\\
...
//-----------------------PROD--------------------------------------\\
		...
		oPr:Say(nLin+385,0100,"Código Peça:",oFonteN1)
	    oPr:Say(nLin+385,1300,"Código Peça:",oFonteN1)
