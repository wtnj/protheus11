...
User Function Nhppa005(lBrow,cPecaAuto,cJPEG)
...
Private oFont16, oFont08, oFont10, oFontCou08, oFontNPR
...
Do While !Eof() .and. QK8->QK8_PECA+QK8->QK8_REV == cPecaRev
	...
	oPrint:Say(lin,0995,QK8->QK8_SEVER,oFontCou08)

	PPAPBMP(QK8->QK8_CLASS+".BMP", cStartPath)
	oPrint:SayBitmap(lin,1029, cStartPath+QK8->QK8_CLASS+".BMP",40,40)

	oPrint:Say(lin,1395,QK8->QK8_OCORR,oFontCou08)
	oPrint:Say(lin,1755,QK8->QK8_DETEC,oFontCou08)

	If Val(QK8->QK8_NPR) >= nNPRMAX
		oPrint:Say(lin,1795,QK8->QK8_NPR,oFontNPR)
	Else
		oPrint:Say(lin,1795,QK8->QK8_NPR,oFontCou08)
	Endif

	oPrint:Say(lin,2195,Subs(QK8->QK8_RESP,1,17),oFontCou08)
	...
	Else
		oPrint:Say(lin,2931,QK8->QK8_RNPR,oFontCou08)
	Endif
	...
	If !Empty(QK8->QK8_CHAVE1)
		...
		If	DbSeek(xFilial("QKO")+"QPPA130A" + QK8->QK8_CHAVE1) // Item Funcao
			Do While !Eof() .and. xFilial("QKO") == QKO->QKO_FILIAL .and. ;
				...
				If !Empty(QKO->QKO_TEXTO) .AND. !Empty(Substr(QKO->QKO_TEXTO,1,10))
		...
		If	DbSeek(xFilial("QKO")+"QPPA130B" + QK8->QK8_CHAVE1) // Modo de Falha Potencial
			Do While !Eof() .and. xFilial("QKO") == QKO->QKO_FILIAL .and. ;
						QKO->QKO_CHAVE == QK8->QK8_CHAVE1 .and.;
						QKO->QKO_ESPEC == "QPPA130B"
				...
				If !Empty(QKO->QKO_TEXTO) .AND. !Empty(Substr(QKO->QKO_TEXTO,1,10))
					...
					Endif
      				...
      				If nPos == 0
						aAdd( aTxt,{	QKO->QKO_SEQ, Space(17), Subs(QKO->QKO_TEXTO,1,17), Space(17),;
										Space(17), Space(8), Space(8), Space(17), Space(17) })
      				Else
						aTxt[nPos,3] := Subs(QKO->QKO_TEXTO,1,17)
      				Endif
				Endif	
				DbSkip()
			Enddo
		...
		If	DbSeek(xFilial("QKO")+"QPPA130C" + QK8->QK8_CHAVE1) // Efeito de Falha Potencial
			Do While !Eof() .and. xFilial("QKO") == QKO->QKO_FILIAL .and. ;
						QKO->QKO_CHAVE == QK8->QK8_CHAVE1 .and.;
						QKO->QKO_ESPEC == "QPPA130C"
				...
				If !Empty(QKO->QKO_TEXTO) .AND. !Empty(Substr(QKO->QKO_TEXTO,1,10))
					...
					Endif
      				...
      				If nPos == 0
						aAdd( aTxt,{	QKO->QKO_SEQ, Space(17), Space(17), Subs(QKO->QKO_TEXTO,1,17),;
										Space(17), Space(8), Space(8), Space(17), Space(17) })
      				Else
						aTxt[nPos,4] := Subs(QKO->QKO_TEXTO,1,17)
      				Endif
				Endif	
				DbSkip()
			Enddo
		...
		If	DbSeek(xFilial("QKO")+"QPPA130D" + QK8->QK8_CHAVE1) // Causa/Mecanismo Potencial da Falha
			Do While !Eof() .and. xFilial("QKO") == QKO->QKO_FILIAL .and. ;
						QKO->QKO_CHAVE == QK8->QK8_CHAVE1 .and.;
						QKO->QKO_ESPEC == "QPPA130D"
				...
				If !Empty(QKO->QKO_TEXTO) .AND. !Empty(Substr(QKO->QKO_TEXTO,1,10))
					...
					Endif
      				...
      				If nPos == 0
						aAdd( aTxt,{	QKO->QKO_SEQ, Space(17), Space(17), Space(17),;
										Subs(QKO->QKO_TEXTO,1,17), Space(8), Space(8), Space(17), Space(17) })
      				Else
						aTxt[nPos,5] := Subs(QKO->QKO_TEXTO,1,17)
      				Endif
				Endif	
				DbSkip()
			Enddo
		...
		If	DbSeek(xFilial("QKO")+"QPPA130E" + QK8->QK8_CHAVE1) // Controles atuais do processo
			Do While !Eof() .and. xFilial("QKO") == QKO->QKO_FILIAL .and. ;
						QKO->QKO_CHAVE == QK8->QK8_CHAVE1 .and.;
						QKO->QKO_ESPEC == "QPPA130E"
				...
				If !Empty(QKO->QKO_TEXTO) .AND. !Empty(Substr(QKO->QKO_TEXTO,1,10))
					...
					Endif
      				...
      				If nPos == 0
						aAdd( aTxt,{	QKO->QKO_SEQ, Space(17), Space(17), Space(17),;
										Space(17), Subs(QKO->QKO_TEXTO,1,8), Space(8), Space(17), Space(17) })
      				Else
						aTxt[nPos,6] := Subs(QKO->QKO_TEXTO,1,8)
      				Endif
				Endif	
				DbSkip()
			Enddo
		...
		If	DbSeek(xFilial("QKO")+"QPPA130H" + QK8->QK8_CHAVE1) // Controles atuais do projeto Deteccao
			Do While !Eof() .and. xFilial("QKO") == QKO->QKO_FILIAL .and. ;
						QKO->QKO_CHAVE == QK8->QK8_CHAVE1 .and.;
						QKO->QKO_ESPEC == "QPPA130H"
				...
				If !Empty(QKO->QKO_TEXTO) .AND. !Empty(Substr(QKO->QKO_TEXTO,1,10))
					...
					Endif
      				...
      				If nPos == 0
						aAdd( aTxt,{	QKO->QKO_SEQ, Space(17), Space(17), Space(17), Space(17),;
										Space(8),Subs(QKO->QKO_TEXTO,1,8), Space(17), Space(17) })
      				Else
						aTxt[nPos,7] := Subs(QKO->QKO_TEXTO,1,8)
      				Endif
				Endif	
				DbSkip()
			Enddo
		...
		If	DbSeek(xFilial("QKO")+"QPPA130F" + QK8->QK8_CHAVE1) // Acoes Recomendadas
			Do While !Eof() .and. xFilial("QKO") == QKO->QKO_FILIAL .and. ;
						QKO->QKO_CHAVE == QK8->QK8_CHAVE1 .and.;
						QKO->QKO_ESPEC == "QPPA130F"
				...
				If !Empty(QKO->QKO_TEXTO) .AND. !Empty(Substr(QKO->QKO_TEXTO,1,10))
					...
					Endif
      				...
      				If nPos == 0
						aAdd( aTxt,{	QKO->QKO_SEQ, Space(17), Space(17), Space(17), Space(17),;
										Space(8), Space(8), Subs(QKO->QKO_TEXTO,1,17), Space(17) })
      				Else
						aTxt[nPos,8] := Subs(QKO->QKO_TEXTO,1,17)
      				Endif
				Endif	
				DbSkip()
			Enddo
		...
		If	DbSeek(xFilial("QKO")+"QPPA130G" + QK8->QK8_CHAVE1) // Acoes Tomadas
			Do While !Eof() .and. xFilial("QKO") == QKO->QKO_FILIAL .and. ;
						QKO->QKO_CHAVE == QK8->QK8_CHAVE1 .and.;
						QKO->QKO_ESPEC == "QPPA130G"
				...
				If !Empty(QKO->QKO_TEXTO) .AND. !Empty(Substr(QKO->QKO_TEXTO,1,10))
					...
					Endif
      				...
      				If nPos == 0
						aAdd( aTxt,{	QKO->QKO_SEQ, Space(17), Space(17), Space(17), Space(17),;
										Space(8), Space(8), Space(17), Subs(QKO->QKO_TEXTO,1,17)})
      				Else
						aTxt[nPos,9] := Subs(QKO->QKO_TEXTO,1,17)
      				Endif
				Endif	
				DbSkip()
			Enddo
	...
	If Len(aTxt) > 0
		For nx := 1 To Len(aTxt)
			//oPrint:Say(lin,0050," AQUI",oFontCou08)
			oPrint:Say(lin,0050,aTxt[nx,2],oFontCou08)
			oPrint:Say(lin,0360,aTxt[nx,3],oFontCou08)
			oPrint:Say(lin,0680,aTxt[nx,4],oFontCou08)
			oPrint:Say(lin,1080,aTxt[nx,5],oFontCou08)
			oPrint:Say(lin,1440,aTxt[nx,6],oFontCou08)
			oPrint:Say(lin,1600,aTxt[nx,7],oFontCou08)
			oPrint:Say(lin,1880,aTxt[nx,8],oFontCou08)
			oPrint:Say(lin,2510,aTxt[nx,9],oFontCou08)
...
oPrint:Say(200,2020,AllTrim(QK7->QK7_PECA)+" "+QK7->QK7_REV,oFontCou08)
...
oPrint:Say(360,2020,QK7->QK7_FMEA,oFontCou08)
...
oPrint:Say(520,0040,Subs(QK7->QK7_OBS,1,100),oFontCou08)
...
// Descricao itens
oPrint:Say(640,100,STR0021,oFont08) //"Funcao /"
oPrint:Say(680,100,STR0022,oFont08) //"Requisitos"
oPrint:Say(720,100,"Processo",oFont08) 
