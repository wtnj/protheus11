...
// WHB Inicio
Local _cPCFor   := Space(10)
Local _cCodFor  := Space(10) 
Local _cCod     := Space(15)
Local _cCodcli := Space(15)
Local _cPC     := Space(10)
// WHB Fim
...
//-- WHB INICIO
SA7->(DbSetOrder(1))  //filial+cliente+loja+produto //Inserido por Alexandre R. Bento - Buscar o codigo do cliente p/ imprimir no xml
		...
		Else
			...
			If MsSeek(xFilial("SF3")+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE)
				...
				If lSimpNac
					...
					Else
						If SF2->F2_VALICM > 0 .And. !Empty(cCfop) .And. Alltrim(SF3->F3_CFO) $ cCfop  // Novo Tratamento
							cMensFis += "Documento emitido por ME ou EPP optante pelo Simples Nacional."
				...
				If lConsig .And. SF3->F3_VALIPI > 0
					If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
						cMensFis += " "
					EndIf
					cMensFis += "Valor do IPI: R$ " + AllTrim(Transform(SF3->F3_VALIPI, "@ze 9,999,999,999,999.99")) + " "
				EndIf
				// Busca valor da reduÁ„o do total da nota, referÍnte ao 
				// decreto RICM, N 43.080/2002 valido somente em MG				
				If SF3->(FieldPos("F3_VL43080")) <> 0 .And. SF3->F3_VL43080 > 0 .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "MG"
					nDescRed += SF3->F3_VL43080
				EndIF				
			EndIf		
			dbSelectArea("SF2")
			//⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒø
			...
			If !Empty(SF2->F2_DUPL)	
				...
				#IFDEF TOP
					...
					BeginSql Alias cAliasSE1
						...
						SELECT E1_FILIAL,E1_PREFIXO,E1_NUM,E1_PARCELA,E1_TIPO,E1_VENCORI,E1_VALOR,E1_VLCRUZ,E1_ORIGEM,E1_PIS,E1_COFINS,E1_CSLL,E1_INSS
						FROM %Table:SE1% SE1
						WHERE
						...
						((SE1.E1_TIPO = %Exp:MVNOTAFIS%) OR
						 ((SE1.E1_ORIGEM IN ('LOJA701','FATA701','LOJA010')) AND SE1.E1_TIPO IN (%Exp:cWhere%))) AND        ////alterado por solicitaÁ„o do Loja
						 SE1.%NotDel%
				...
				While !Eof() .And. xFilial("SE1") == (cAliasSE1)->E1_FILIAL .And.;
					...
					SF2->F2_DOC == (cAliasSE1)->E1_NUM     
						If (cAliasSE1)->E1_TIPO = MVNOTAFIS .OR. ((AllTrim((cAliasSE1)->E1_ORIGEM) $ 'LOJA701|FATA701|LOJA010') .AND. (cAliasSE1)->E1_TIPO $ cWhere)       //alterado por solicitaÁ„o do Loja
							If cValLiqB == "1"
								aadd(aDupl,{(cAliasSE1)->E1_PREFIXO+(cAliasSE1)->E1_NUM+(cAliasSE1)->E1_PARCELA,(cAliasSE1)->E1_VENCORI;
								,((cAliasSE1)->E1_VLCRUZ-(cAliasSE1)->E1_PIS-(cAliasSE1)->E1_COFINS-(cAliasSE1)->E1_CSLL-(cAliasSE1)->E1_INSS)})					
							Else
								aadd(aDupl,{(cAliasSE1)->E1_PREFIXO+(cAliasSE1)->E1_NUM+(cAliasSE1)->E1_PARCELA,(cAliasSE1)->E1_VENCORI,(cAliasSE1)->E1_VLCRUZ})	
							EndIf
						EndIf
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD2
						...
						D2_QUANT,D2_TOTAL,D2_DESCON,D2_VALFRE,D2_SEGURO,D2_PEDIDO,D2_ITEMPV,D2_DESPESA,D2_VALBRUT,D2_VALISS,D2_PRUNIT,/* WHB Inicio*/D2_TP,D2_TES,/* WHB Fim */
						...
						D2_ORIGLAN,D2_VALCF3,D2_VALIPI %Exp:cField% 
						FROM %Table:SD2% SD2
						WHERE
			...
			While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				If lNfCup
					...
					DbSelectArea("SD2")
					DbSetOrder(3)
					If DbSeek(xFilial("SD2")+cNumNfCup+cSerNfCup+cCliNota+cCliLoja+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEM)
						cD2Cfop := SD2->D2_CF
						cD2Tes	:= SD2->D2_TES
					EndIf
					RestArea(aAreaSD2)
				...
				If !Empty((cAliasSD2)->D2_NFORI) 
					If (cAliasSD2)->D2_TIPO $ "DBN"
						...
						If MsSeek(xFilial("SD1")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
							...
							//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ
							If AllTrim(SF1->F1_ESPECIE)<>"NFP"
								If cChave <> (SD1->D1_EMISSAO,SD1->D1_SERIE,SD1->D1_DOC,IIF(SD1->D1_TIPO $ "DB",IIF(SD1->D1_FORMUL=="S",SM0->M0_CGC,SA1->A1_CGC),IIF(SD1->D1_FORMUL=="S",SM0->M0_CGC,SA2->A2_CGC)),SM0->M0_ESTCOB,SF1->F1_ESPECIE,SF1->F1_CHVNFE) .OR. (cAliasSD2)->D2_ITEM <> cItemOr
									...
									cChave := (SD1->D1_EMISSAO,SD1->D1_SERIE,SD1->D1_DOC,IIF(SD1->D1_TIPO $ "DB",IIF(SD1->D1_FORMUL=="S",SM0->M0_CGC,SA1->A1_CGC),IIF(SD1->D1_FORMUL=="S",SM0->M0_CGC,SA2->A2_CGC)),SM0->M0_ESTCOB,SF1->F1_ESPECIE,SF1->F1_CHVNFE)
								EndIf	
					   		...
					   		If MsSeek(xFilial("SFT")+"S"+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI)
					   			If !Empty(SFT->FT_DTCANC)
							   		...
							   		If Empty(SF3->F3_CODRSEF) .Or. SF3->F3_CODRSEF == "101"
						   				If cChave <> (SF3->F3_EMISSAO,SF3->F3_SERIE,SF3->F3_NFISCAL,SA1->A1_CGC,SM0->M0_ESTCOB,SF3->F3_ESPECIE,SF3->F3_CHVNFE) .OR. (cAliasSD2)->D2_ITEM <> cItemOr
											aadd(aNfVinc,{SF3->F3_EMISSAO,SF3->F3_SERIE,SF3->F3_NFISCAL,SA1->A1_CGC,SM0->M0_ESTCOB,SF3->F3_ESPECIE,SF3->F3_CHVNFE})
											cChave := (SF3->F3_EMISSAO,SF3->F3_SERIE,SF3->F3_NFISCAL,SA1->A1_CGC,SM0->M0_ESTCOB,SF3->F3_ESPECIE,SF3->F3_CHVNFE)
										EndIf 
										cItemOr:=(cAliasSD2)->D2_ITEM
									EndIf
								EndIf
...
//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
				...
				If !Empty(SF4->F4_FORMULA) .And. Formula(SF4->F4_FORMULA) <> NIL .And. ( ( cMVNFEMSF4=="C" .And. !AllTrim(Formula(SF4->F4_FORMULA)) $ cMensCli ) .Or. (cMVNFEMSF4=="F" .And. !AllTrim(Formula(SF4->F4_FORMULA))$cMensFis) )
					...
					EndIf
				Else // WHB Inicio incluido p/ trazer a descriÁ„o fiscal na danfe
				...
				If lNfCup 		
					nDesconto := Round((cAliasSD2)->D2_QUANT*(cAliasSD2)->D2_PRUNIT,2)-(cAliasSD2)->D2_TOTAL
	            Else 
		        ...
		        If !Empty((cAliasSD2)->D2_IDENTB6) 
		        	If SC5->C5_TIPO == "N" 
				     ...
				     ElseIf SC5->C5_TIPO == "B"
				         ...
				         SA5->(dbSetOrder(1)) 	         
				         If SA5->(MsSeek( xFilial("SA5") + (cAliasSD2)->(D2_CLIENTE+D2_LOJA+D2_COD) ))
				...
				If Subs(cCodProd,11,1) == "." .Or. Subs(cCodProd,13,1) == "." .Or. Subs(cCodProd,1,2) == "EC" .Or. (cAliasSD2)->D2_TP == "PA" .Or. (cAliasSD2)->D2_TIPO$"N/C/I/P" 
	         
		           IF SA7->(DbSeek(xFilial("SA7")+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+cCodProd)) 
			          ...
			          _cPCFor  := Alltrim(SA7->A7_PCCLI)  // Numero do pedido do cliente na WHB (New Hubner)
			 	      _cCodFor := Alltrim(SA7->A7_CODFOR)  // Codigo da New Hubner no Cliente
  			          IF !Empty(Alltrim(SA7->A7_PCCLI)) /*.And. (cAliasSD2)->D2_CLIENTE$"900025/900022/900171/900024"*/ .And. ((cAliasSD2)->D2_TP == "PA" .Or. (cAliasSD2)->D2_TES == "578") //gera a tag XML (NPED, NITENS) somente p/ PA.  
			             aadd(aPedCom,{Alltrim(SA7->A7_PCCLI),1}) //nf saida pesquisar SA7 numero do pedido + item do pedido
            		      ...
            		      If (cAliasSD2)->D2_TP == "PA" .Or. (cAliasSD2)->D2_TES == "578"//Para tratar a TAG InfAdProd
            		         ...
            		         If (cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA== "90002201" //Cliente IVECO gera TAG com MOTORS
             		            cInfAdic := "IAP01"+ Iif(len(_cCodcli)>11, _cCodcli+Space(30-len(_cCodcli)),_cCodcli+Space(19))+_cPC+"MOTORES   "
             		         Else
             		            cInfAdic := "IAP01"+ Iif(len(_cCodcli)>11, _cCodcli+Space(30-len(_cCodcli)),_cCodcli+Space(19))+_cPC
            		         Endif
			 	...
			 	Else
                   aadd(aPedCom,{})                  
				   _cCod    := cCodProd //atribui o codigo do produto                   
			 	Endif       
				...
				aadd(aProd,	{Len(aProd)+1,;
					...
					SB1->B1_UM,;
					(cAliasSD2)->D2_QUANT,;
					IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR,0),;
					...
					(cAliasSD2)->D2_SEGURO,;
					(nDesconto),;
					IIF(!(cAliasSD2)->D2_TIPO$"IP",NoRound( ( (cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR )/(cAliasSD2)->D2_QUANT,8),0 ),;
					...
					(cAliasSD2)->D2_NUMLOTE,;//Numero do Lote
				   	IIF((cAliasSD2)->D2_TIPO == "D" .And. !lIpiDev .Or. lConsig ,(cAliasSD2)->D2_DESPESA + (cAliasSD2)->D2_VALPS3 + (cAliasSD2)->D2_VALCF3 + (cAliasSD2)->D2_VALIPI , (cAliasSD2)->D2_DESPESA + (cAliasSD2)->D2_VALPS3 + (cAliasSD2)->D2_VALCF3),;      //Outras despesas + PISST + COFINSST  (Inclus„o do valor de PIS ST e COFINS ST na tag vOutros - NT 2011/004).E devoluÁÁ„o com IPI.
			    ...
			    If SFT->(DbSeek(xFilial("SFT")+"S"+(cAliasSD2)->(D2_SERIE+D2_DOC+D2_CLIENTE+D2_LOJA+PadR(D2_ITEM,TamSx3("FT_ITEM")[1])+D2_COD)))
				   ...
				   If SFT->(FieldPos("FT_PRINCMG")) > 0 .And. SFT->(FieldPos("FT_VLINCMG")) > 0
					...
					EndIf
				EndIf 
				If SFT->FT_ESTADO=="AM" .And. SFT->FT_CLASFIS=="040" .And. SFT->FT_DESCZFR>0 
					...
					Do Case
						...
						Case AllTrim(CD2->CD2_IMP) == "IPI"
							If !lConsig
								...
								If !lIpiDev
									If ( (cAliasSD2)->D2_TIPO=="D" .And. !Empty(nValIPI) ).OR. ( (cAliasSD2)->D2_TIPO=="P" .And. lComplDev .And. !Empty(nValIPI) )
				...
				//Tratamento para que o valor de PIS ST e COFINS ST venha a compor o valor total da tag vOutros  (NT 2011/004). E devoluÁÁ„o com IPI n„o tributado
				If ((cAliasSD2)->D2_TIPO == "D" .AND. !lIpiDev)  .Or. lConsig
					aTotal[01] += (cAliasSD2)->D2_DESPESA + (cAliasSD2)->D2_VALPS3 + (cAliasSD2)->D2_VALCF3 + (cAliasSD2)->D2_VALIPI
				Else 
					aTotal[01] += (cAliasSD2)->D2_DESPESA + (cAliasSD2)->D2_VALPS3 + (cAliasSD2)->D2_VALCF3 
				EndIf
				...
				If SFT->(FieldPos("FT_CSTPIS")) > 0 .And. SFT->(FieldPos("FT_CSTCOF")) > 0
					...
					If MsSeek(xFilial("SFT")+"S"+SF2->F2_SERIE+SF2->F2_DOC+SF2->F2_CLIENTE+SF2->F2_LOJA+PadR((cAliasSD2)->D2_ITEM,4)+(cAliasSD2)->D2_COD)
						...
						IF Empty(aPis[Len(aPis)]) .And. (SFT->FT_CSTPIS$"09,06,04,99" .Or. SFT->FT_CSTPIS$"70")
							aTail(aPisAlqZ):= {SFT->FT_CSTPIS}
						EndIf
						IF Empty(aCOFINS[Len(aCOFINS)]) .And. (SFT->FT_CSTCOF$"09,06,04,99" .Or. SFT->FT_CSTCOF$"70")
				...
				Else
					...
					IF Empty(aPis[Len(aPis)]) .And. (SF4->F4_CSTPIS$"09,06,04,99" .Or. SF4->F4_CSTPIS$"70")
						aTail(aPisAlqZ):= {SF4->F4_CSTPIS}		
					EndIf
					IF Empty(aCOFINS[Len(aCOFINS)]) .And. (SF4->F4_CSTCOF$"09,06,04,99" .Or. SF4->F4_CSTCOF$"70")
				...
				dbSkip()
		    EndDo
		     //Retira o desconto referÍnte ao RICMS 43080/2002
		    If nDescRed > 0                                           				
				aTotal[2] -= nDescRed 
		    EndIF 
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			If MsSeek(xFilial("SF3")+SF1->F1_FORNECE+SF1->F1_LOJA+SF1->F1_DOC+SF1->F1_SERIE)
   				// Busca valor da reduÁ„o do total da nota, para referÍnte ao 
				// decreto RICM, N 43.080/2002 valido somente em MG				
				If SF3->(FieldPos("F3_VL43080")) <> 0 .And. SF3->F3_VL43080 > 0 .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "MG"
					nDescRed += SF3->F3_VL43080
				EndIF
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD1			
						...
						D1_NUMLOTE,/* WHB INICIO */ D1_DESCRI, /* WHB FIM */D1_CUSTO,D1_ORIGLAN,D1_DESCICM,D1_II,D1_FORMUL,D1_VALPS3,D1_ORIGLAN,D1_VALCF3 %Exp:cField%
						FROM %Table:SD1% SD1
						WHERE
				...
				If !Empty((cAliasSD1)->D1_NFORI) 
					...
					If MsSeek(xFilial("SD1")+(cAliasSD1)->D1_NFORI+(cAliasSD1)->D1_SERIORI+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+(cAliasSD1)->D1_COD+(cAliasSD1)->D1_ITEMORI)
						...
						Else
							...
							MsSeek(xFilial("SA2")+SD1->D1_FORNECE+SD1->D1_LOJA)
						EndIf
				        lRural := AllTrim(SF1->F1_ESPECIE) == "NFP" 
					...
					If !(cAliasSD1)->D1_TIPO $ "DBN" .Or. lRural
						...
						//≥    da tabela SA2.                                                                            ≥
						//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ
						If AllTrim(SF1->F1_ESPECIE)=="NFP" .and. lSeekOk
							//para nota de entrada tipo devolucao o emitente eh o cliente ou o sigamat no caso de formulario proprio=sim
							If SD1->D1_TIPO$"DB"
								aadd(aNfVincRur,{SD1->D1_EMISSAO,SD1->D1_SERIE,SD1->D1_DOC,SF1->F1_ESPECIE,;
									IIF(SD1->D1_FORMUL=="S",SM0->M0_CGC,SA1->A1_CGC),; 
							...
							dbSelectArea("SFT")
					   		...
					   		If MsSeek(xFilial("SFT")+"E"+(cAliasSD1)->D1_NFORI+(cAliasSD1)->D1_SERIORI)
					   			If !Empty(SFT->FT_DTCANC)
							   		...
							   		MsSeek(xFilial("SF3")+SFT->FT_CLIEFOR+SFT->FT_LOJA+SFT->FT_NFISCAL+SFT->FT_SERIE)
							   		If Empty(SF3->F3_CODRSEF) .Or. SF3->F3_CODRSEF == "101"
						   				If cChave <> (SF3->F3_EMISSAO,SF3->F3_SERIE,SF3->F3_NFISCAL,SA2->A2_CGC,SM0->M0_ESTCOB,SF3->F3_ESPECIE,SF3->F3_CHVNFE) .OR. (cAliasSD1)->D1_ITEM <> cItemOr
											aadd(aNfVinc,{SF3->F3_EMISSAO,SF3->F3_SERIE,SF3->F3_NFISCAL,SA2->A2_CGC,SM0->M0_ESTCOB,SF3->F3_ESPECIE,SF3->F3_CHVNFE})
											cChave := (SF3->F3_EMISSAO,SF3->F3_SERIE,SF3->F3_NFISCAL,SA2->A2_CGC,SM0->M0_ESTCOB,SF3->F3_ESPECIE,SF3->F3_CHVNFE)
										EndIf 
										cItemOr:=(cAliasSD1)->D1_ITEM
									EndIf
								EndIf
...
If !Empty(aNota)
	...
	For nX := 1 To Len(aProd)
		cString += NfeItem(aProd[nX],aICMS[nX],aICMSST[nX],aIPI[nX],aPIS[nX],aPISST[nX],aCOFINS[nX],aCOFINSST[nX],aISSQN[nX],aCST[nX],aMed[nX],aArma[nX],aveicProd[nX],aDI[nX],aAdi[nX],aExp[nX],aPisAlqZ[nX],aCofAlqZ[nX],aAnfI[nX],cTipo,cVerAmb, aComb[Nx],@cMensFis,aCsosn[Nx],aPedCom[nX],aNota[5],aICMSZFM[nX])
	Next nX
  	cString += NfeTotal(aTotal,aRetido,aICMS,aICMSST)
...
If cVerAmb== "2.00" 
	Do Case
		Case (!Empty(aNfVinc) .And. !(aNota[5]$"NDB") .And. SF4->F4_AJUSTE <> "S")
	  		cTPNota:= "2"
		Case ((aNota[5]) $ "I-D" .And. SF4->F4_AJUSTE == "S")
	   		cTPNota:= "3"
		OtherWise 
	  		cTPNota:= "1"
		EndCase
...
Static Function NfeDest(aDest,cVerAmb,aTransp,aCST)
	...
	If ConvType(aDest[17]) == "2" .And. ConvType(aDest[14]) <> "ISENTO" .And. SuperGetMV("MV_ESTADO") <> "SP"
		cString += '<IE>'+""+'</IE>'
	Else
		cString += '<IE>'+ConvType(aDest[14])+'</IE>'
	Endif
	
	//Tratamento para o Suframa, se o produto for 0- Nacional, o mesmo envia a tag <IESUF>, 1-Importado ou 2-Nacionalizado NAO.
	If !Empty(aDest[15])
		For nX:= 1 to Len(aCST)
			If !(aCST[1][2])$"12"
...
Static Function NfeItem(aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ,aAnfI,cTipo,cVerAmb,aComb,cMensFis,cCsosn,aPedCom,cF2Tipo,aICMSZFM)
...
If  !lIssQn    
	If cMVCODREG == "1" .and. cVerAmb == "2.00" .and. SF4->(FieldPos("F4_CSOSN"))>0 .And. !Empty(SF4->F4_CSOSN)
		...
		If cCsosn$"500" 
			If aCST[01] = "60" .AND. cTipo=="1"	
				SPEDRastro(aProd[20],aProd[19],aProd[02],@nBaseIcm,@nValICM)      
		 		If nBaseIcm > 0
		...
		If aCST[01] = "60" .AND. cTipo=="1"		
			...
			If nBaseIcm > 0 .and. nValICM>0
			    ...
			    ElseIF SF4->F4_ART274 == "1"  .And. Upper(SM0->M0_ESTCOB) == "PR"  
					nBaseIcm := aProd[09]*nBaseIcm
					nValICM  := aProd[09]*nValICM
					cMensFis += "Imposto Recolhido por SubstituiÁ„o - Artigo 271 do RICMS (Par·grafo 1o, alÌnea B, inciso II, onde o 'Cod.Produto:  " +ConvType(aProd[02])+" ' Valor da Base de ST: R$ " +str(nBaseIcm,15,2)+" Valor de ICMS ST: R$ "+str(nValICM,15,2)+" "
				Else
	...
	ElseIf Len(aCSTIPI) > 0
		...
		cString += '</cpl>'
		cString += '<CST>'+ConvType(aCSTIPI[01][1])+'</CST>'
		cString += '<modBC>'+ConvType(3)+'</modBC>'
		cString += '<pRedBC>'+ConvType(0,5,2)+'</pRedBC>'
		cString += '<vBC>'  +ConvType(0,15,2)+'</vBC>'
		cString += '<aliquota>'+ConvType(0,5,2)+'</aliquota>'
...
Else
	...
	If (lSimpNac .And. ( !Empty(aPisAlqZ) .And. aPisAlqZ[01] == "99" )) .Or. ( !Empty(aPisAlqZ) .And. aPisAlqZ[01] == "70" )
		cString += '<CST>'+aPisAlqZ[01]+'</CST>'
	Else
		cString += '<CST>'+(IIF(Len(aPisAlqZ)>0 .And. aPisAlqZ[01]$"09,06,04,07",aPisAlqZ[01],"08"))+'</CST>'
	EndIf
	cString += '<modBC></modBC>'
	cString += '<pRedBC></pRedBC>'
	cString += '<vBC>'+ConvType(0,15,2)+'</vBC>'
...
Else
	...
	If (lSimpNac .And. ( !Empty(aCofAlqZ) .And. aCofAlqZ[01] == "99" )) .Or. ( !Empty(aCofAlqZ) .And. aCofAlqZ[01] == "70" )
		cString += '<CST>'+aCofAlqZ[01]+'</CST>'
	Else
		cString += '<CST>'+(IIF(Len(aCofAlqZ)>0 .And. aCofAlqZ[01]$"09,06,04,07",aCofAlqZ[01],"08"))+'</CST>'
	EndIf                       
...
cString += '<infadprod>'+ConvType(aProd[25],500)+'</infadprod>'

cString += '</det>'
Return(cString)

Static Function NfeTotal(aTotal,aRet,aICMS,aICMSST)
...
cString += '<Cpl>[ContrTSS='+StrZero(Year(ddatabase),4)+'-'+StrZero(Month(ddatabase),2)+'-'+StrZero(Day(ddatabase),2)+'#'+AllTrim(Time())+'#'+upper(AllTrim(cUserName))+']'
...
If Len( aNfVinc ) > 0 .Or. Len( aNfVincRur ) > 0 
...
Next
If nValII > 0
	cString += ("Valor total do Imposto de ImportaÁao : R$ " + ConvType(nValII,15,2)+ " .O valor do Imposto de ImportaÁao nao est· embutido no valor dos produtos, somente ao valor total da NF-e.")
Endif
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÑo    ≥SPEDRastro≥ Autor ≥ Luana Ferrari         ≥ Data ≥13/10/2009≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÑo ≥Funcao criada para retornar para matxfis a informacao da UF ≥±±
...
ﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂ
/*/
Static Function SPEDRastro(cNumLote,cLoteCtl,cProduto,nBaseIcm,nValICM)
...
If Len (cSeek)>0 .AND. nBaseIcm==0 .AND. nValICM==0 .And. SF4->F4_ART274 == "1" 
	...
	BeginSql Alias cAliasSFT
		SELECT FT_BASERET,FT_ICMSRET,FT_PRODUTO
		FROM %Table:SFT% SFT
		WHERE
		SFT.FT_FILIAL = %xFilial:SFT% AND
		SFT.%NotDel% AND 
		FT_NFISCAL =%Exp:SB8->B8_DOC % AND
		FT_SERIE   =%Exp:SB8->B8_SERIE% AND
		FT_TIPOMOV =%Exp:"E" % AND
		FT_CLIEFOR =%Exp:SB8->B8_CLIFOR% AND
		FT_LOJA	   =%Exp:SB8->B8_LOJA %AND
		FT_PRODUTO =%Exp:cProduto%
	EndSql
		
	If (cAliasSFT)->(!Eof()) .And. (cAliasSFT)->FT_BASERET > 0 .And. (cAliasSFT)->FT_ICMSRET > 0
